...
DATA_SECTION
 ...
 !! cout<<"SMS version 20 October 2011 using ADMB version 10.0"<<endl;
 ...
 init_matrix tmp(1,nsp,1,9)// various species information on "input" format
 ...
 !! }
 !! cout<<"SSB_Rec_model:"<<SSB_Rec_model<<endl;
 ...
 !! for (s=first_VPA;s<=nsp;s++) {
 !!   if (SSB_year_first(s)<=fy)  SSB_year_first(s)=fy;
 ...
 init_matrix obj_weight(1,nsp,1,4)       // 1=catch observations, 
                                         // 2=CPUE observations,
                                         // 3=SSB/R relations
 ...
 init_number min_catch_CV    // minimum accepted CV of catch at age form commercial catches
 ...
 init_int seasonal_annual_catches        // use seasonal or annual catches in objective function
 ...
 !! for (s=first_VPA;s<=nsp;s++) catch_sep_year(s,1)=fy; //First year are always the overall assessment first year
 ...
 !! for (i=1;i<4;i++) if  (est_calc_sigma(i)!=1 &&  est_calc_sigma(i)!=0 &&  est_calc_sigma(i)!=2) {
 ...
 !! if (mceval==1 && (read_HCR_file==0 || read_HCR_file==2)) {
 ...
 !! int stomach_samples=1;   // proxy for sampling level. No. of hauls 
 ...
 !!   if (size_selection(s)==0 || size_selection(s)==4 || size_selection(s)==11) Stom_var_fac(s)=1.0; else Stom_var_fac(s)=0.1;
 !! }
 ...
 //!! if (read_HCR_file==2) ad_comm::change_datafile_name("short-term-configuration.dat");
 ...
 !! else ad_comm::change_datafile_name("just_one.in");
 init_3darray zero_catch_y_season(first_VPA,nsp,fy,lyData,fq,lq)
 ...
 !! if (seasonal_annual_catches==1) ad_comm::change_datafile_name("F_q_ini.in"); 
 !! else ad_comm::change_datafile_name("just_one.in");
 init_matrix F_q_ini_read(1,no_sp_sag_syg,fcq,lcq)
 //!! cout<<"F_q_ini_read:"<<endl<<F_q_ini_read<<endl;


  //  starts to process HCR.dat file
 !! if (read_HCR_file==0 || read_HCR_file==2 ) ad_comm::change_datafile_name("just_one.in");
 !! else if (read_HCR_file==1) ad_comm::change_datafile_name("HCR_options.dat");
  
  int pnsp            // prediction, number of species, normally nsp
 !! if (read_HCR_file==1 ) pnsp=nsp; else if (read_HCR_file==0 ||read_HCR_file==2 ) pnsp=1; 
   
  init_int lpy        // last year in prediction
 !! if (read_HCR_file==0 || read_HCR_file==2 ) lpy=lyModel+1; 
 !!  if (test_output==51) cout <<"Last.prediction.year: "<< lpy<<endl;
 
  init_int no_MCMC_iterations       // no of iterartions within each MCMC prediction
 ...
 init_ivector first_year_propmat(first_VPA,pnsp)    // first year for calc of prediction proportionmature
 ...
 !!  if (rec_noise_trunc_a(s)>= rec_noise_trunc_b(s)  && (read_HCR_file==1)) {
 ...
 init_vector rec_noise_input(first_VPA,pnsp)
  ...
  init_matrix tmp2(1,46,first_VPA,pnsp)// various HCR information by species on "input" format
 ...
 !!     TAC_cap(s)              =tmp2(15,s);
 !!     F_cap(s)                =tmp2(16,s);
 !!     F_constraint(s,1)       =tmp2(17,s);
 !!     F_constraint(s,2)       =tmp2(18,s);
 !!     TAC_constraint(s,1)     =tmp2(19,s);
 !!     TAC_constraint(s,2)     =tmp2(20,s);
 !!     SSB_constraint(s,1)      =tmp2(21,s);
 !!     SSB_constraint(s,2)      =tmp2(22,s); 
 !!     obs_noise_trunc(s,1)     =tmp2(23,s);
 !!     obs_noise_trunc(s,2)     =tmp2(24,s);
 
 !!     real_time_uncertanty(s,1)=tmp2(25,s);
 !!     real_time_uncertanty(s,2)=tmp2(26,s);
 !!     real_time_uncertanty(s,3)=tmp2(27,s);
 !!     real_time_uncertanty(s,4)=tmp2(28,s);

 !!     survey_uncertanty(s,1)   =tmp2(29,s);
 !!     survey_uncertanty(s,2)   =tmp2(30,s);
 !!     survey_uncertanty(s,3)   =tmp2(31,s);
 !!     survey_uncertanty(s,4)   =tmp2(32,s);

 !!     assess_uncertanty(s,1)   =tmp2(33,s);
 !!     assess_uncertanty(s,2)   =tmp2(34,s);
 !!     assess_uncertanty(s,3)   =tmp2(35,s);
 !!     assess_uncertanty(s,4)   =tmp2(36,s);
  
 !!     implemt_uncertanty(s,1)  =tmp2(37,s);
 !!     implemt_uncertanty(s,2)  =tmp2(38,s);
 !!     implemt_uncertanty(s,3)  =tmp2(39,s);
 !!     implemt_uncertanty(s,4)  =tmp2(40,s);

 !!     TAC_first(s)             =tmp2(41,s);
 !!     TAC_second(s)            =tmp2(42,s);
 !!     F_first(s)               =tmp2(43,s);
 !!     F_second(s)              =tmp2(44,s);
 !!     interYear(s)             =int(tmp2(45,s));
 !!     inter_F_TAC(s)           =tmp2(46,s);
 !! }
 ...
 !! if (read_HCR_file==0 || read_HCR_file==2) read_SSB_R=0; 
 ...
 !! else { ad_comm::change_datafile_name("covariance_N.in"); tmpA=fa;}
  init_3darray coVariance(first_VPA,nsp,fa+1,max_a,tmpA+1,la_VPA);
 ...
 !! local_lpy=lpy+2;
 !! if (use_rec_noise_input==0 || mceval==0) {
 !!   local_lpy=lyModel-1;
 !!    ad_comm::change_datafile_name("just_one.in");
 !! }
 ...
 !!  cout<<"input explotation pattern:"<<endl<<setprecision(3)<<endl<<init_exploitation_pattern<<endl;
 !! }
 ...
 int sf
 matrix fleet_effort(1,no_sp_fl,v_first_fleet_year,v_last_fleet_year)     //effort by species, fleet and age
 ...
 init_3darray effort(first_VPA,tmpE,fq,lq,fy,lyData) 
 !! if (any_do_effort!=0 && test_output==2) cout<<"Observed total effort:"<<endl<<effort<<endl;
 // Normalise to mean effort=1                                            
 number sum_effort
 !! if (any_do_effort==1) {
 !!   for (s=first_VPA;s<=nsp;s++) if (do_effort(s)==1) {
 !!     sum_effort=0;
 !!     for (q=fq;q<=lq;q++)  sum_effort+=sum(effort(s,q));
 !!     sum_effort=sum_effort/(lyData-fy+1);
 !!     for (q=fq;q<=lq;q++) for (y=fy;y<=lyData;y++) effort(s,q,y)=effort(s,q,y)/sum_effort;
 !! }}                      
 ...
 LOCAL_CALCS
  ...
  #define CALC_yq yq=(y-fy)*lq+q;
  #define CALC_yqd yqd=(y-fy)*q_d+(q-fq)*no_areas+d;  // cout<<"y:"<<y<<" q:"<<q<<" d:"<<d<<" yqd:"<<yqd<<endl;

  // macroes to transform data from input format (by species) to format by timestep
  // without area
  #define MOVE(in,out,first,last) \
   { for(y=fy;y<=lyModel;y++) for(q=fq;q<=lq;q++)  {  \
     CALC_yq \
  ...
  #define MOVEd(in,out,first,last) \
   { for(y=fy;y<=lyData;y++) for(q=fq;q<=lq;q++) for(d=1;d<=no_areas;d++) {  \
     CALC_yqd   \
   ...
   #define change_input_multi(in) if (multi>=1) ad_comm::change_datafile_name(in); if (test_output==2 && multi>=1) cout<<"Begin reading file:"<<in<<endl;
 ...
 !! for (y=fy;y<=lyModel;y++) for (q=fq;q<=lq;q++) for (d=1;d<=no_areas;d++)  {
 !!  CALC_yqd 
...
!! for (y=fy;y<=lyModel+1;y++) for (q=fq;q<=lq;q++) {
 !!  i=(y-fy)*lq+q;
 !!  vAll_fa1(i)=faq(q);
 !! }
 ...
 ivector vys1(fy,lyModel)   // year-species
 ivector vys2(fy,lyModel)   // year-species
 !! vys1=first_VPA;
 !! vys2=nsp;
 imatrix vVPA_lay(fy,lyModel,first_VPA,nsp)       // last age VPA species, for used in ragged array for VPA species
 imatrix vVPA_fay(fy,lyModel,first_VPA,nsp)
 !! for (i=fy;i<=lyModel;i++) for (s=first_VPA;s<=nsp;s++) {vVPA_fay(i,s)=fa; vVPA_lay(i,s)=la(s);}
 ...
 init_4darray obs_C_input(first_VPA,nsp,fy,lyData,fq,lq,fa,max_a) 
 !! if (test_output==2) cout<<"Observed catch numbers from file canum.in:"<<endl<<obs_C_input<<endl;
 !! if (seasonal_annual_catches==1 && lq>1 && recq>1) { //add all catches into recruitment quarter, such that nothing is lost in the MOVE, where informationfrom nonrecruited ages are deleted 
 !!   for (s=first_VPA;s<=nsp;s++) for (y=fy;y<=lyModel;y++) for (q=fq;q<recq;q++) for (a=fa;a<=la(s);a++)  {
 !!     if (obs_C_input(s,y,q,a)>0) obs_C_input(s,y,recq,a)+=obs_C_input(s,y,q,a);
 !!     obs_C_input(s,y,q,a)=0;
 !!   }
 !!   for (s=first_VPA;s<=nsp;s++) for (y=fy;y<=lyModel;y++) for (q=recq+1;q<=lq;q++) for (a=fa;a<=la(s);a++) {
 !!     if (obs_C_input(s,y,q,a)>0) obs_C_input(s,y,recq,a)+=obs_C_input(s,y,q,a);
 !!     obs_C_input(s,y,q,a)=0;
 !!   }
 !! }
 !! // for (s=first_VPA;s<=nsp;s++) cout<<"CANUM: s"<<s<<endl <<obs_C_input<<endl;
 3darray obs_C(1,yq,first_VPA,v_last_sp,vVPA_fa,vVPA_la);
 !! MOVE(obs_C_input,obs_C,first_VPA,nsp);
 
  //********************************************************************************************* 
 // M, single species Natural mortalities
 !! change_input("natmor.in");
 init_4darray M_input(first_VPA,nsp,fy,lyData,fq,lq,fa,max_a) 
 !! if (test_output==2) cout<<"Natural mortalities from file natmor.in:"<<endl<<M_input<<endl;
 3darray M(1,yq,first_VPA,v_last_sp,vVPA_fa,vVPA_la)
 !! MOVE(M_input,M,first_VPA,nsp);
 
 //********************************************************************************************* 
 // Mean weight in the sea
 !! change_input("west.in");
 init_4darray west_input(1,nsp,fy,lyData,fq,lq,fa,max_a) 
 !! if (test_output==2) cout<<"Weight in the stock from file west.in:"<<endl<<west_input<<endl;
 3darray west(1,yq,1,v_last_sp,vAll_fa,vAll_la)
 !! MOVE(west_input,west,1,nsp);
       
 //********************************************************************************************* 
 // Mean weight in the catch
 !! change_input("weca.in");
 init_4darray weca_input(first_VPA,nsp,fy,lyData,fq,lq,fa,max_a) 
 !! if (test_output==2) cout<<"Weight in the catch from file weca.in:"<<endl<<weca_input<<endl;
 3darray weca(1,yq,first_VPA,v_last_sp,vVPA_fa,vVPA_la)
 !! MOVE(weca_input,weca,first_VPA,nsp);

 //********************************************************************************************* 
 // Proportion mature
 !! change_input("propmat.in");
 !! if (any_do_effort) y=lyData+1; else y=lyData;
 init_4darray propmat_input(first_VPA,nsp,fy,y,fq,lq,fa,max_a) 
 !! if (test_output==2) cout<<"proportion mature from file propmat.in:"<<endl<<propmat_input<<endl;
 3darray propmat(1,yq1,first_VPA,v_last_sp1,vVPA_fa1,vVPA_la1)
 !! if (any_do_effort) k=lyData+1; else k=lyData;
 !! for(y=fy;y<=k;y++) for(q=fq;q<=lq;q++)  {  
 !!    i=(y-fy)*lq+q;  
 !!    for (s=first_VPA;s<=nsp;s++) for(a=faq(q);a<=la(s);a++) propmat(i,s,a)=propmat_input(s,y,q,a);
 !! }
 !! 
 //*********************************************************************************************
...
init_matrix known_recruitment(first_VPA,nsp,fy,lyData);
...
!! if (test_output==2) cout<< "known_recruitment:"<<endl<<known_recruitment<<endl;
 ...
 !! if (multi==0) ad_comm::change_datafile_name("just_one.in");
  ...
  3darray log_obs_C_annual(fy,lyModel,first_VPA,vys2,vVPA_fay,vVPA_lay)   // log of observed catches (annual catches)
  3darray log_obs_C(1,yq,first_VPA,v_last_sp,fa,vVPA_la)   // log of observed catches (seasonal catches)
 ...
 int Myq;
 !! if (multi==0) {
 !!   Mnsp=first_VPA; Mnpr=1; Mnopr=1; Mly=fy; Mlq=fq; Mmax_a=fa; Myq=1;
 ...
 init_5darray overlap(1,no_areas,1,Mnpr,fy,Mly,fq,Mlq,0,Mindex)
 ...
 !!  if (use_overlap==0 && multi>0) for(d=1;d<=no_areas;d++) for(s=1;s<=Mnpr;s++) for(y=fy;y<=Mly;y++) overlap(d,s,y)=1.0;
 ...
 !! if (multi>=1) change_input_multi("consum.in");

 !! if (multi==0) Mindex=-1; else Mindex=max_a;
 init_5darray consum_input(1,no_areas,1,Mnpr,fy,Mly,fq,Mlq,fa,Mindex)
 !! if (test_output==3 && multi>0) cout<<"Consum (ration) from file consum.in:"<<endl<<consum_input<<endl;
 3darray consum(1,def_yqd,1,yqdPred,yqdsPred_fa,yqdsPred_la)
 !! if (multi>0) MOVEd(consum_input,consum,1,npr);

 //********************************************************************************************* 
  // Proportion of the stock number by area,
 
 !! if (multi>=1  && no_areas>1) change_input_multi("stock_distribution.in");
 !! if (multi==0) Mindex=-1; else Mindex=max_a;
 init_5darray N_dist_input(1,no_areas,1,Mnsp,fy,lyData,fq,Mlq,fa,Mindex)
 !! if (test_output==3 && multi>0 && no_areas>1) cout<<"Stock distribution from file Stock_distribution.in:"<<endl<<N_dist_input<<endl;
 3darray N_dist(1,def_yqd,1,yqdAll,yqdsAll_fa,yqdsAll_la)
 !! if (multi>0) MOVEd(N_dist_input,N_dist,1,nsp);
 
 //********************************************************************************************* 
 !! if (multi==0 || consum_op==0) Mindex=-1; else {Mindex=lq; ad_comm::change_datafile_name("consum_ab.in");}
 init_matrix consum_a(1,Mnpr,fq,Mindex)
 !! if (multi==0 || consum_op==0) Mindex=-1; else Mindex=Mnpr;
 init_vector consum_b(1,Mindex)
 !! if (test_output==3 && multi>0 && consum_op>0) cout<<"Consumption parameters from file consum_ab.in:"<<endl<<setprecision(3)<<consum_a<<endl<<consum_b<<endl;
 ...
 !! if (multi==0) Mindex=-1; else Mindex=max_a;
 init_5darray lsea_input(1,no_areas,1,Mnsp,fy,lyData,fq,Mlq,fa,Mindex)
 ...
 imatrix index_Lq(fy,lyModel,fq,lq)      // pointer to the first species for a given year and quarter
 ...
 !! if (test_output==3 && multi>0) cout<<"Relaltive stomach contents from file stomcon_at_length.in:"<<endl<<stl_stom<<endl;
 ...
 imatrix  stl_stom_use_prey(1,n_stl_yqdpl,1,l_stl_yqdplpl);   // make use of the stomach observations for a prey species  for calc of available food?
 ...
 init_4darray incl_stom(1,Mnpr,fq,lq,mil,mal,1,n_stl_y);  //Include stomach sample observations in likelihood 0=no inlusion, >=1 include data
 4darray incl_stom_out(1,Mnpr,fq,lq,mil,mal,1,n_stl_y);  // copy of above, for output purposes
 ...
 !! if (incl_stom_all==0){
 !!  for (p=1;p<=Mnpr;p++) for (q=fq;q<=lq;q++) { incl_stom(p,q)=1; incl_stom_out(p,q)=0; }
 !! }
  !! if (incl_stom_all==1){
 !!  for (p=1;p<=Mnpr;p++) for (q=fq;q<=lq;q++) { incl_stom_out(p,q)=incl_stom(p,q); }
 !! }
 ...
 !!   }
 !! }  
 !! else if (stomach_samples==3) {
 !!   for (spl=1;spl<=n_stl_yqdpl;spl++) { 
 !!     for (splp=1;splp<=l_stl_yqdplpl(spl);splp++) {
 !!       stl_no_samples(spl,splp)=1; //same number in all
 !!     }  
 !!   }
 !! }
 ...
 int sd;
 LOCAL_CALCS
  for (s=1;s<=Mnpr;s++)  min_no_samples(s)=1000;
      ...
      for (sq=stl_y(sy,2);sq<=stl_y(sy,3);sq++) { 
        ...
        for (sd=stl_yq(sq,2);sd<=stl_yq(sq,3);sd++) { 
          ...
          for (sp=stl_yqd(sd,2);sp<=stl_yqd(sd,3);sp++) {
             s=stl_yqdp(sp,1);
             //cout<<"predator: "<<s<<endl;
             for (spl=stl_yqdp(sp,2);spl<=stl_yqdp(sp,3);spl++) {
                //cout<<"predator length:"<<stl_yqdpl(spl,1)<<endl; 
                int ll=0; 
                if (incl_stom_all==0) incl_stom_out(stl_yqdp(sp,1),stl_yq(sq,1),stl_yqdpl(spl,1),sy)= stl_no_samples(spl,1);
                for (splp=stl_yqdpl(spl,2);splp<=stl_yqdpl(spl,3);splp++) {
                  ...
                  for (l=stl_yqdplp(splp,2);l<=stl_yqdplp(splp,3);l++) {
                    //cout<< l<<" ";
                    ll++;
                    if (incl_stom(stl_yqdp(sp,1),stl_yq(sq,1),stl_yqdpl(spl,1),sy) >0) if (stl_no_samples(spl,ll)<min_no_samples(s)) min_no_samples(s)=stl_no_samples(spl,ll); 
 ...
 !! } //if (multi>=1) 
  ...
  !!  res<<"# file incl_stom.out "<<endl<<"# A value >=1 indicate that the stomach sample is used in SMS. Default number is no. of hauls"<<endl;
  !!  for (p=1;p<=Mnpr;p++) {
  ...
  !!     for (i=mil;i<=mal;i++) {
  !!      for (j=1;j<=n_stl_y;j++) {
  !!        res<< setw(6)<<incl_stom_out(p,q,i,j);
  !!      }
  !!      res<<" # Size: "<<i<<endl; 
  !!   }}}
  !! res.close();
 ...
 !! else ad_comm::change_datafile_name("just_one.in");
 !! if (multi==0) Mindex=-1; else Mindex=max_a;
 init_4darray other_pred_N_input(1,Mnopr,fy,Mly,fq,Mlq,fa,Mmax_a) 
 ...
 !! if (multi>0) MOVE(other_pred_N_input,other_pred_N,1,nOthPred);
 ...
 !! if (test_output>=1) cout << endl << "Input of all data completed" << endl;
 ...
 4darray              tmp_SSB(1,ndim,1,no_MCMC_iterations,first_VPA,nsp,fy,lpy);
 4darray    tmp_SSB_percieved(1,ndim,1,no_MCMC_iterations,first_VPA,nsp,lyModel+1,lpy);
 4darray              tmp_TSB(1,ndim,1,no_MCMC_iterations,first_VPA,nsp,fy,lpy);
 4darray           tmp_mean_F(1,ndim,1,no_MCMC_iterations,first_VPA,nsp,fy,lpy);
 4darray tmp_mean_F_percieved(1,ndim,1,no_MCMC_iterations,first_VPA,nsp,fy,lpy);
 4darray            tmp_yield(1,ndim,1,no_MCMC_iterations,first_VPA,nsp,fy,lpy);
 4darray         tmp_eaten_M2(1,ndim,1,no_MCMC_iterations,first_VPA,nsp,fy,lpy);
 4darray          tmp_recruit(1,ndim,1,no_MCMC_iterations,first_VPA,nsp,fy,lpy);

 int sdReportYear;
 !! sdReportYear=min(5,lyModel-fy);        // number of years in SD report for mean F and SSB
...
PARAMETER_SECTION
 ...
 //  Recruits first age
 init_bounded_matrix log_rec(first_VPA,nsp,fy,lyModel,-15.,10.,phase_log_rec)
 !! if  (phase_log_rec>0) {
 !!   for (s=first_VPA;s<=nsp;s++) for (y=fy;y<=lyModel;y++)
 ...
 !!   parNo++; parexp<<"log_rec_older "<<parNo<<" "<<s<<" "<<fy<<" "<<fq<<" -1 "<<a<<" -1 -1 -1"<<endl;} 
 !! }

  3darray F_a(first_VPA,nsp,fy,lyModel,fa,la_VPA)             //age selection in separable model
  matrix F_y(first_VPA,nsp,fy,lyModel)                          //year selection in separable model
 ...
 !! for (s=first_VPA;s<=nsp;s++) if (Nknots(s)==1) {lly(s)=lyModel; Nknots(s)=0;} else lly(s)=fy;
 ...
 // Year effect in Fishing mortality. F_y(s,fy)=1 is constant
 init_bounded_matrix F_y_ini(first_VPA,nsp_Fy,fy+1,lly,0.01,15.0,phase_F_y_ini)
 !! if (phase_F_y_ini>0 ) {           
 !!   for (s=first_VPA;s<=nsp;s++) if (do_effort(s)==0) for (y=fy+1;y<=lly(s);y++) {
 !!    parNo++;
 ...
 // Seasonal effect in fishing mortality. F_q(s,lq) is constant 1 for annaul VPA or 1/nseason for seasonal VPA  
 !! if (seasonal_annual_catches==1) { phase_F_q_ini=-1; no_sp_sag_syg=-1;}
 init_bounded_matrix F_q_ini(1,no_sp_sag_syg,fcq,lcq,0.0001,3E3,phase_F_q_ini)
 ...
 !!      for (syg=1;syg<=n_catch_sep_year_group(s);syg++) {
 !!       no_sp_sag_syg++;
 !!       for (q=fcq(no_sp_sag_syg);q<=lcq(no_sp_sag_syg);q++) { 
 !!          parNo++;
 ...
 matrix obj_func(1,nsp,1,5)    // objective function contribution (Catch, CPUE, SSB/R, stomach, penalty) by species

 //catchability by species, fleet and age
 init_bounded_matrix qq_ini(1,no_sp_fl,v_first_fleet_age,v_last_fleet_age_q,1E-4,1E4,phase_qq_ini)
 !! if (phase_qq_ini>0) {
 !!   int no_sp_fl=0;       
 !!   for (s=first_VPA;s<=nsp;s++) for (f=1;f<=n_fleet(s);f++){ 
 !!     no_sp_fl++;
 ...
 !! if (phase_SSB_R_beta>0 && est_calc_sigma(3)==0)  for (s=first_VPA;s<=nsp;s++) if (use_beta_SSB_Rec(s)==1) {parNo++;  parexp<<"SSB_R_beta "<<parNo<<" "<<s<<" -1 -1 -1 -1 -1 -1 -1"<<endl; }
 ...
 3darray   C_hat_annual(fy,lyModel,first_VPA,vys2,vVPA_fay,vVPA_lay)   // Expected catch annual

 3darray         M2(1,yq,first_VPA,v_last_sp,vVPA_fa,vVPA_la)
 3darray          F(1,yq,first_VPA,v_last_sp,vVPA_fa,vVPA_la)
 3darray          Z(1,yq,first_VPA,v_last_sp,vVPA_fa,vVPA_la)
 3darray          N(1,yq1,1,v_last_sp1,vAll_fa1,vAll_la1)   // stock number at the start of the season
 ...
 3darray   avail_food(1,yq,1,v_last_Psp,vpr_fa,vpr_la)      // available food
 ...
 3darray   other_bio(1,Mnopr,fy,lpy,fq,lq)      // Biomass of other predators
 
 matrix    TSB(first_VPA,nsp,fy,lpy)           // Total Stock Biomass, first season
 matrix    SSB(first_VPA,nsp,fy,lpy)           // Spawning Stock Biomass, first season
 matrix    SSB_percieved(first_VPA,nsp,lyModel+1,lpy)           // Percieved Spawning Stock Biomass, first season
 matrix    yield(first_VPA,nsp,fy,lpy)         // Yield
 matrix    yield_hat(first_VPA,nsp,fy,lpy)     // expected Yield
 matrix    eaten_M2(first_VPA,nsp,fy,lpy)      // eaten biomass
 matrix    Mean_F(first_VPA,nsp,fy,lpy)        // Mean  F (real)
 matrix    Mean_F_percieved(first_VPA,nsp,fy,lpy)    // Mean  F (percieved)
...
//!! double spl,spu; //lower and upper value for size preference parameter
 ...
 !! if (size_select_model==1)      {spl=1.0; spu=5.0; }  //length
 ...
 !!   else if (size_selection(s)==1 || size_selection(s)==11) n++;
...
//lower and upper value for size preference parameter variance
 ...
 !!   Stom_var_u(s)= 50.0*Stom_var_l(s);
 !!  }
 !! }
 !! if (multi==0) n=0; else n=Mnpr;
 ...
 vector stl_avail_food(1,n_stl_yqdpl)                   // available food for a predator-length
 ...
 !! int na_sp=sum(la_VPA-fa);
 !! if (fq==recq) na_sp=na_sp+nsp-first_VPA+1;
 ...
 // !! for (s=first_VPA;s<=nsp;s++) for (a=fa+1;a<=la_VPA(s);a++)  {parNo++;  parexp<<"term_N_next "<<parNo<<" "<<s<<" "<<lyModel+1<<" "<<fq<<" -1 "<<a<<"  -1 -1 -1"<<endl; }
 ...
 //sdreport_vector log_term_N_next(1,na_sp)
 //!! for (s=first_VPA;s<=nsp;s++) for (a=fa+1;a<=la_VPA(s);a++) {parNo++;  parexp<<"log_term_N_next "<<parNo<<" "<<s<<" "<<lyModel+1<<" "<<fq<<" -1 "<<a<<"  -1 -1 -1"<<endl; }
  vector log_term_N_next(1,na_sp)
 
 // Fishing mortality at age, last assessment year 
 !! int na_spF=0;
 !! for (s=first_VPA;s<=nsp;s++) for (a=cfa(s);a<=las(s);a++) for (q=fq;q<=lq;q++) if (incl_catch_season_age(s,q,a)>0) na_spF++;
 //sdreport_vector term_F(1,na_spF)
 //!! for (s=first_VPA;s<=nsp;s++) for (a=cfa(s);a<=las(s);a++) for (q=fq;q<=lq;q++) if (incl_catch_season_age(s,q,a)>0){parNo++;  parexp<<"term_F "<<parNo<<" "<<s<<" "<<lyModel<<" "<<q<<" -1 "<<a<<"  -1 -1 -1"<<endl; }
 ...
 matrix M2_sd0(1,nprey,fy,lyModel)
 // sdreport_matrix M2_sd0(1,nprey,fy,lyModel)
 ...
 matrix M2_sd1(1,nprey,fy,lyModel)
 //sdreport_matrix M2_sd1(1,nprey,fy,lyModel)
 ...
 matrix M2_sd2(1,nprey,fy,lyModel)
 //sdreport_matrix M2_sd2(1,nprey,fy,lyModel)
...
PRELIMINARY_CALCS_SECTION
 ...
 cout << "PRELIMINARY_CALCS_SECTION begins"<<endl;
  ...
  // Separable model, set first year (and all the remaning yeras) , year effect to 1 
  F_y=1.0;
  
  
   // set F_q_ini to 1/(no of seasons) as initial guess
   if (seasonal_annual_catches==0) {
   if (mean(F_q_ini)==1) {               // if FALSE read in of parameters from external files (done automatically)
    int sp_sag_syg=0;
    for (s=first_VPA;s<=nsp;s++){
    for (sag=1;sag<=n_catch_season_age_group(s);sag++) {    
      for (syg=1;syg<=n_catch_sep_year_group(s);syg++) {            
         sp_sag_syg++;
         a=catch_season_age(s,sag);
         for(q=fq_F(s,a);q<lq_F(s,a);q++) {  F_q_ini(sp_sag_syg,q)=fix_F_factor(s)/(lq_F(s,a)-fq_F(s,a)+1);}
        }
     }
    }
  }}
  ...
  for (y=fy;y<=lyModel;y++) log_obs_C_annual(y)=0;
  for (s=first_VPA;s<=nsp;s++){
    nobs=0;
    sum_C(s)=0.0;
    sum_C_annual(s)=0.0;
    for (y=fy;y<=lyModel;y++){
      ...
      for (q=fq;q<=lqLocal;q++) {
         ...
         for (a=cfa(s);a<=la(s);a++) if (!(a==fa && q<recq)) {
          ...
          yield(s,y)+=weca(yq,s,a)*obs_C(yq,s,a);
          sum_C(s,q,a)+=obs_C(yq,s,a);
          sum_C_annual(s,a)+=obs_C(yq,s,a);
          if (a<=la_like(s)) {
            if (seasonal_annual_catches==1 && q==fq) nobs++;
            else if (seasonal_annual_catches==0) nobs++;
          }
        }
      }
    }  
  ...
  for (s=first_VPA;s<=nsp;s++) no_obj_obs(s,3)=SSB_year_last(s)-SSB_year_first(s)+1;   
  ...
  if (seasonal_annual_catches==1) {
   for (s=first_VPA;s<=nsp;s++){
     for (y=fy;y<=lyModel;y++){
      for (a=fa;a<=la(s);a++) {
        if (log_obs_C_annual(y,s,a)<=0) {
           tmp=-sum_C_annual(s,a)/(lyModel-fy+1)*min_catch(s)/100.0;   // minimum catch ;
      ...
      CALC_yq
         for (a=cfa(s);a<=la(s);a++) if (!(a==fa && q<recq)) {
          ...
          if (min_catch(s)>0) {  
            ...
            if (sum_C(s,q,a)>0) tmp=-sum_C(s,q,a)/(lyModel-fy+1)*min_catch(s)/100.0;
            else tmp=1.0;
            tmp=tmp*exp(0.2*randn(rnd));
            if ((obs_C(yq,s,a)==0) && (!((a==fa) && (q <recq)))) {
            ...
            if (sum_C(s,q,a)>0) tmp=-sum_C(s,q,a)/(lyModel-fy+1)*min_catch(s)/10000.0;
            else tmp=1.0;
            tmp=tmp*exp(0.2*randn(rnd));
            if ((obs_C(yq,s,a)<tmp) && (!((a==fa) && (q <recq)))) {
              if (mceval==0 && test_output==1) cout<<"catch "<<obs_C(yq,s,a)<<" substituted by "<<tmp<<" s:"<<species_names[s]<<" y:"<<y<<" q:"<<q<<" a:"<<a<<endl;
              log_obs_C(yq,s,a)=log(tmp);
            }
   ...
   for (y=fy;y<=lyModel;y++) {
     ...
     for (q=fq;q<=lqLocal;q++) {
      ...
      for (a=faq(q);a<=la(s);a++) {
         if (consum_op==1) {
           if (west(yq,s,a)>0) consum(yqd,s,a)=consum_a(s,q)* pow(west(yq,s,a),consum_b(s));
           else consum(yqd,s,a)=0;
         }  
         else {
            if (lsea(yqd,s,a)>0) consum(yqd,s,a)=consum_a(s,q)* pow( L_W_ab(s,1)* pow(lsea(yqd,s,a),L_W_ab(s,2)),consum_b(s));
           else consum(yqd,s,a)=0;
        }
      }
     }
         ...
         for (La=ALKS_yqds(Ls,2);La<=ALKS_yqds(Ls,3);La++) {                                                                   
           ...
           for (Ll=ALKS_yqdsa(La,2);Ll<=ALKS_yqdsa(La,3);Ll++) {
             ...
             if (s<=npr) {
               ...
               else consum_l(yqd,s,a,Ll)= consum_a(s,q)* pow(size_l_sea(yqd,s,a,Ll),consum_b(s));
             }
           }
         } 
 }}}}}  
 ...
 // classify each stomach observation: used or not used? 
 if (mceval==0) for (sy=1;sy<=n_stl_y;sy++) { 
   for (sq=stl_y(sy,2);sq<=stl_y(sy,3);sq++) { 
    for (sd=stl_yq(sq,2);sd<=stl_yq(sq,3);sd++) { 
     for (sp=stl_yqd(sd,2);sp<=stl_yqd(sd,3);sp++) {
       pred=stl_yqdp(sp,1);
        for (spl=stl_yqdp(sp,2);spl<=stl_yqdp(sp,3);spl++) {
         ...
         for (splp=stl_yqdpl(spl,2);splp<=stl_yqdpl(spl,3);splp++) {
           prey=stl_yqdplp(splp,1);
           sumPrey=0; noPrey=0;  //ny
           for(iprey_l=stl_yqdplp(splp,2);iprey_l<=stl_yqdplp(splp,3);iprey_l++) {
              ll++;
              noPrey++;   // ny  
              if (prey>0) {
                if (stl_stom(spl,ll)<min_stom_cont(pred)) stl_stom_use_like(spl,ll)=0;        // exclude too small stomach contents
                else  if  (prey_size(spl,ll)/pred_size(spl)>prey_pred_size_fac(pred))  stl_stom_use_like(spl,ll)=0;      // exclude too large preys       
              }
              sumPrey=sumPrey+stl_stom(spl,ll); //ny
           ...
           ////
            if (prey>0) {
             ...
             for(iprey_l=stl_yqdplp(splp,2);iprey_l<=stl_yqdplp(splp,3);iprey_l++) {
                ll++;  
                if (sumPrey<min_stom_cont(pred)) { 
                  stl_stom_use_like(spl,ll)=0;      // exclude observations for all size classes of a prey species where the total stom contents are below threshold  
                  stl_stom_use_prey(spl,ll)=0; 
                  if (noPrey==ll) cout <<"Prey:"<<species_names[prey]<<" excluded from stomach observation for predator:"<<species_names[pred]<< 
                     " size:"<<stl_yqdpl(spl,4)<<" Y:"<<stl_y(sy,1)<<" Q:"<<stl_yq(sq,1)<<" due to prey species sum <"<<min_stom_cont(pred)<<endl;    
                }
              }
             }
          }
  ...
  stl_stom_use_avail=stl_stom_use_like;
         ...
         pred=stl_yqdp(sp,1);
          for (spl=stl_yqdp(sp,2);spl<=stl_yqdp(sp,3);spl++) {
           ...
           for (splp=stl_yqdpl(spl,2);splp<=stl_yqdpl(spl,3);splp++) {
             ...
             for(iprey_l=stl_yqdplp(splp,2);iprey_l<=stl_yqdplp(splp,3);iprey_l++) {
                ll++;      
                if (prey>0 && stl_stom(spl,ll)>=min_stom_cont(pred)){
    ...
    for (sq=stl_y(sy,2);sq<=stl_y(sy,3);sq++) { 
      ...
      for (sp=stl_yqd(sd,2);sp<=stl_yqd(sd,3);sp++) {
        ...
        for (spl=stl_yqdp(sp,2);spl<=stl_yqdp(sp,3);spl++) {
          ll=0; 
          used_stom=0;
         if (incl_stom(pred,stl_yq(sq,1),stl_yqdpl(spl,1),sy)>=1) { //Include stomach observations in likelihood 0=no inlusion, >=1 include data
          ...
          else if (mceval!=1) { cout<<"pred:"<<pred<<", "<<species_names[pred]<<" year:"<<stl_y(sy,1)<<" Q:"<<stl_yq(sq,1)<<" pred length class:"<<stl_yqdpl(spl,1)<<
             " pred length group:"<<stl_yqdpl(spl,4)<<" not included, as specified in input file incl_stom.in"<<endl;  }
        }
       }
     }
    }
 ...
 for (pred=1;pred<=npr;pred++) if (size_selection(pred)==0 || size_selection(pred)==4 || size_selection(pred)==11)  stoms(2,pred)=0;
 if (mceval==0) for (sy=1;sy<=n_stl_y;sy++) {
   ...
   for (sq=stl_y(sy,2);sq<=stl_y(sy,3);sq++) {
     ...
     for (sd=stl_yq(sq,2);sd<=stl_yq(sq,3);sd++) { 
      for (sp=stl_yqd(sd,2);sp<=stl_yqd(sd,3);sp++) {
        ...
        if (size_selection(pred)==0 || size_selection(pred)==4 || size_selection(pred)==11) {  // no size (uniform) selection
           used_stom=0;
           for (spl=stl_yqdp(sp,2);spl<=stl_yqdp(sp,3);spl++) {
           ll=0;
           //pred_l=int(pred_length(spl));
           for (splp=stl_yqdpl(spl,2);splp<=stl_yqdpl(spl,3);splp++) {
             ...
             for(iprey_l=stl_yqdplp(splp,2);iprey_l<=stl_yqdplp(splp,3);iprey_l++) {
               ...
               if (prey==0) used_stom++;
               if (prey>0) {
                 if (iprey_l==stl_yqdplp(splp,2)) {  // first observation for a prey species
                   first_ll=ll; 
                   max_samples=stl_no_samples(spl,ll);
                   used_stom++;
                   //if (stl_stom_use_like(spl,ll)==0) cout<<"skal være 1: Y:"<<stl_y(sy,1)<<" Q:"<<stl_yq(sq,1)<<" pred:"<<pred<<" Pred L:"<<int(pred_length(spl))<<" prey:"<<prey<<" prey L:"<<iprey_l<<endl;;
                   if  (stl_stom_use_prey(spl,first_ll)==1)  {        // the sum of stomach contents for a given prey species is above limit
                      stl_stom_use_like(spl,first_ll)=1;  stl_stom_use_avail(spl,first_ll)=1; 
                   } 
                 }
                 else  { // second or higher observation for a prey species
                   stl_stom(spl,first_ll)=stl_stom(spl,first_ll)+stl_stom(spl,ll);
                   stl_stom(spl,ll)=0.0;
                   stl_stom_use_like(spl,ll)=0;  
                   stl_stom_use_avail(spl,ll)=1;
 ...
 if (9==9 && mceval==0) {
   ...
   res<<"Prey.length.class Prey.length.mean Prey.weight Prey.size size.ratio N.haul stomcon stomcon.input use_like use_avail use_prey ";
         ...
         for (spl=stl_yqdp(sp,2);spl<=stl_yqdp(sp,3);spl++) {
           ...
           for (splp=stl_yqdpl(spl,2);splp<=stl_yqdpl(spl,3);splp++) {
             ...
             for(prey_l=stl_yqdplp(splp,2);prey_l<=stl_yqdplp(splp,3);prey_l++) {
               ...
               res <<stl_stom(spl,ll)<<" "<<stl_stom_input(spl,ll)<<" "<<stl_stom_use_like(spl,ll)<<" "<<stl_stom_use_avail(spl,ll)<<" "<<stl_stom_use_prey(spl,ll)<<" ";
  ...
  for (y=fy;y<=lyModel;y++) for (q=fq;q<=lq;q++) { 
   ...
   if (first_VPA>1) for (s=1;s<first_VPA;s++) for (y=fy;y<=lyModel;y++) for (q=fq;q<=lq;q++) {
          CALC_yq
  ...
  if (def_pred_par==1) { 
    ...
    if (stomach_variance==1 || stomach_variance==2) for (s=1;s<=npr;s++) Stom_var(s)=1.05; 
    else if (stomach_variance==3) for (s=1;s<=npr;s++) Stom_var(s)=2/min_no_samples(s)/Stom_var_fac(s);
    ...
    for (s=1;s<=npr;s++) { 
       if (size_selection(s)!=0 && size_selection(s)!=4) {
        n++;
        if (size_selection(s)==1 || size_selection(s)==11 || size_selection(s)==2 || size_selection(s)==3 ) {
  ...
  if (mceval==1) { 
    ...
    obj_func_file<<"Repetion Species.n obj.catch obj.survey obj.SR obj.stom obj.penalty obj.all"<<endl;
  ...
  max_last_VPA=nsp;   // used for testing; selects af sub-set of species
   ...
   // testing
  //min_first_VPA=17; // used for testing; selects af sub-set of species
  //max_last_VPA=24;   // used for testing; selects af sub-set of species
...
PROCEDURE_SECTION 
 ...
 int iter,s,y,q,a;
 int yqMaster;
 dvar_matrix old_M2(first_VPA,nsp,fa,max_a);
 dvariable tmp;
 //check_print_par();
 Rec_parm_adm();
 ...
 for (y=fy;y<=lyModel;y++) {
   lqLocal=(y==lyModel)?lqly:lq; 
   for (q=fq;q<=lqLocal;q++) {
     yqMaster=(y-fy)*lq+q;    // master counter on year quarter index
      ...
      if (use_Nbar==0) {                    // Use N in the beginning of a period to calc M2
         ...
         while ((iter<=max_M2_iteration)) {
           ...
           if ((current_phase()>=stom_phase) && (multi>=2)){
             for (s=first_VPA;s<=nsp;s++) if (is_prey(s)) for (a=faq(s);a<=la(s);a++) old_M2(s,a)=M2(yqMaster,s,a);
             calc_M2(y,q);
             for(s=first_VPA;s<=nsp;s++) if (is_prey(s)) for (a=faq(s);a<=la(s);a++) tmp+=square(M2(yqMaster,s,a)-old_M2(s,a));
           }    
         } // end while loop
 ...
 //if (sd_phase() && read_HCR_file==2) make_short_term_forecast();
 
 //test_prof= pref_size_ratio(1);
...
FUNCTION void calc_M2_simple(int q, int d, dvector other_food, dmatrix size, dmatrix prey_w, dmatrix consum, dvar_matrix N_bar, dvar_matrix& avail_food, dvar_matrix& M2);
...
FUNCTION void calc_M2_one_area(int y, int q);
 ...
 CALC_yqd
 if (simple_ALK==0) {
   calc_M2_simple(q,d,AV_other_food(d),size_sea(yqd),size_sea_prey_w(yqd),consum(yqd),N_bar_stom(yq),avail_food(yq),M2(yq));
...
FUNCTION void calc_M2_many_areas(int y, int q);
 ...
 for (d=1;d<=no_areas;d++) {
   ...
   if (simple_ALK==0) {
     calc_M2_simple(q,d,AV_other_food(d),size_sea(yqd),size_sea_prey_w(yqd),consum(yqd),Nd,avail_food(yq),M2d);
   }
   else {   
       for (s=1;s<=nsp;s++) size(s)=size_l_sea(yqd,s);
       for (pred=1;pred<=npr;pred++) consum_local(pred)=consum_l(yqd,pred);
   ...
   for (pred=1;pred<=npr;pred++){
     ...
     for (pred_a=faq(q);pred_a<=la(pred);pred_a++) {
      ...
      if ( (pred>=first_VPA)|| (pred<first_VPA && N_bar_stom(yq,pred,pred_a)>0)) { 
        ...
        tmp=N_bar_stom(yq,pred,pred_a)*consum(yqd,pred,pred_a)/avail_food(yq,pred,pred_a);
        //cout<<" tmp:"<<tmp<<endl;
        for (prey=first_VPA;prey<=nsp;prey++) {
...
FUNCTION dvariable suit(int y, int q, int d, int pred,int prey,double pred_size,double prey_size, int observed)
 ...
 if (size_selection(pred)==0) {         // (uniform) no size selection
  ...
  if (ratio >= min_pred_prey_size_ratio(pred,prey) &&  ratio <= max_pred_prey_size_ratio(pred,prey)){    
    ...
    if (size_selection(pred)==1  || size_selection(pred)==11 || size_selection(pred)==2 ) { //normal distribution or assymmetric normal distribution
...
FUNCTION dvariable other_suit(int pred,double pred_size,int y, int q, int d)
 dvariable tmp;
 //cout<<"pred: "<<pred<<" "<< AV_other_food_size(pred)<<endl;
...
FUNCTION N_at_length_like
 ...
 // cout <<endl<<"stl_N_bar:"<<endl<<setw(11)<<setprecision(0)<<setfixed()<<stl_N_bar<<endl;
 //********************************************************************************************* 
       ...
       for (sp=stl_yqd(sd,2);sp<=stl_yqd(sd,3);sp++) {
         ...
         for (spl=stl_yqdp(sp,2);spl<=stl_yqdp(sp,3);spl++) {
           ...
           for (splp=stl_yqdpl(spl,2);splp<=stl_yqdpl(spl,3);splp++) {
             ...
             for (iprey_l=stl_yqdplp(splp,2);iprey_l<=stl_yqdplp(splp,3);iprey_l++) {
               ll++;
               if (prey>0 && iprey_l==stl_yqdplp(splp,2)) first_ll=ll;      
               if (prey>0){ 
                 prey_s=prey_size(spl,ll);
                 if ( (size_select_model==1) || (size_select_model==2) || (size_select_model==5)) wstom=stl_wstom(spl,ll);
                 else wstom=prey_s;
                
                 if (size_selection(pred)!=0 && size_selection(pred)!=11 && size_selection(pred)!=4) {    // prey size selection
                   if (stl_stom_use_avail(spl,ll)==1) {
                    stl_prey_avail(spl,ll)=stl_N_bar(spl,ll)*wstom*suit(y,q,d,pred, prey, pred_s,prey_s,observed);
                      if (stl_prey_avail(spl,ll)==0) {
                        ...
                        cout<<"Number prey at length:"<<stl_N_bar(spl,ll)<<" mean weight:"<<wstom<<" suitability "<<suit(y,q,d,pred, prey, pred_s,prey_s,observed)<<endl;;
                      }
                 ...
                 else {   // no prey size selection
                  //cout<<"y:"<<y<<" q:"<<q<<" pred:"<<pred<<" prey:"<< prey<<" pred_s:"<< pred_s<<" prey_s:"<<prey_s<<endl;
                  //cout<<stl_N_bar(spl,ll)<<" "<<wstom<<"  "<<suit(y,q,pred, prey, pred_s,prey_s,observed)<<endl;;
         
                   if (stl_stom_use_avail(spl,ll)==1) tmp=stl_N_bar(spl,ll)*wstom*suit(y,q,d,pred, prey, pred_s,prey_s,observed);
                   else tmp=0;
                   if (ll==first_ll) { 
                     stl_prey_avail(spl,first_ll)=tmp; 
                     stl_prey_avail_part(spl,first_ll)=tmp; 
                   }
                   else {        // add within the full size range
                     stl_prey_avail(spl,first_ll)+=tmp;
                     stl_prey_avail(spl,ll)=0; 
                     stl_prey_avail_part(spl,ll)=tmp; 
                   }
                   stl_avail_food(spl)+=tmp;
                 }       // end no prey size selection
                
               }
               else if (prey==0) {
                 stl_prey_avail(spl,ll)=AV_other_food(d,pred)*other_suit(pred,pred_s,y,q,d);  // Othet food 
                 stl_prey_avail_part(spl,ll)=stl_prey_avail(spl,ll);
                 stl_avail_food(spl)+=stl_prey_avail(spl,ll); 
              }
           }
         ...
         // We have now available food by prey length and total available food by pred length
         ll=0;
         for (splp=stl_yqdpl(spl,2);splp<=stl_yqdpl(spl,3);splp++) {
           for (iprey_l=stl_yqdplp(splp,2);iprey_l<=stl_yqdplp(splp,3);iprey_l++) {
              ll++;
  ...
  // reorganise Other food size dependency
   ...
   if  (!(sd_phase()))  {
    if ((test_output==13 || test_output==19) && active(vulnera)) { 
     ...
     cout <<"Other food suitability slope: "<<active(init_stl_other_suit_slope)<<endl<<init_stl_other_suit_slope<<endl;;
     ...
     cout <<"init_season_overlap:" <<active(init_season_overlap)<< init_season_overlap;
     cout << endl<<"Stom_var:" <<endl<<Stom_var;
     cout <<endl;
    }
   }
  }
...
FUNCTION calc_F
 ...
 int ly_group,syg,syg_fy,syg_ly;
 ...
 // Copy F_y              
  for (s=first_VPA;s<=nsp;s++) if (do_effort(s)==0) {
    if (Nknots(s)>1) {   // splines are used
      ...
      for (y=fy+1;y<=lyModel;y++) F_y(s,y)=splinefn(y);
    } else {  // one value per year
      // copy F_y_ini to F_y
      for (y=fy+1;y<=lyModel;y++) {
        F_y(s,y)=F_y_ini(F_y_species_index(s),y);  // F_y(s,fy) is constant 1;}
        for(syg=1;syg<=n_catch_sep_year_group(s);syg++) {
 ...
 if (lq>fq) { // only seasonal data
    ...
    for (s=first_VPA;s<=nsp;s++){
     for (sag=1;sag<=n_catch_season_age_group(s);sag++) {
      ...
      for (syg=1;syg<=n_catch_sep_year_group(s);syg++) {                
         sp_sag_syg++;
         if (seasonal_annual_catches==0)     { for(q=fq_F(s,a);q<lq_F(s,a);q++) F_q(s,sag,syg,q)=F_q_ini(sp_sag_syg,q);}
         else if (seasonal_annual_catches==1){ for(q=fq_F(s,a);q<lq_F(s,a);q++) F_q(s,sag,syg,q)=F_q_ini_read(sp_sag_syg,q);}
 ...
 /*
 for (s=first_VPA;s<=nsp;s++){
   cout<<"Species:"<<s<<" "<<F_a(s,fy,fa)<<endl;
 } cout<<endl;
 */
 ...
 // calculate  TSB  and SSB
  ...
  for (s=first_VPA;s<=nsp;s++){ 
    for (y=fy;y<=lyModel;y++){
      CALC_yq
      for (a=faq(q);a<=la(s);a++) {
        if ((a==fa) && (fq==recq)) tmp=N(yq,s,a)*west(yq,s,a);     
        else if (a>fa) tmp=N(yq,s,a)*west(yq,s,a);
        else tmp=0.0;
        TSB(s,y)+=tmp;
...
FUNCTION get_expected_catch_at_age
 ...
 // for (yq=1;yq<=(lyModel-fy+1)*lq;yq++) for (s=first_VPA;s<=nsp;s++) C_hat(yq,s)=elem_prod(F(yq,s),N_bar(yq,s));
 ...
 if (seasonal_annual_catches==1) {  // add seasonal catches, if needed
   for (y=fy;y<=lyModel;y++) {
     C_hat_annual(y)=0;
     for (q=fq;q<=lq;q++) {
       CALC_yq
       for (s=first_VPA;s<=nsp;s++) if (zero_catch_y_season(s,y,q)==1) for (a=faq(q);a<=la(s);a++) C_hat_annual(y,s,a)+=C_hat(yq,s,a);
     }
    }
  }
 ...
 for (s=first_VPA;s<=nsp;s++){
   ...
   s2_common=1;
    ...
    for (s2_group=1;s2_group<=n_catch_s2_group(s);s2_group++) {
     ...
     //cout<<"s2_fa:"<<s2_fa<<"  s2_la:"<<s2_la<<endl;

     for (y=fy;y<=lyModel;y++) {
        if (seasonal_annual_catches==0) for (q=fq;q<=lq;q++) {
          if (zero_catch_y_season(s,y,q)==1){
           CALC_yq          
 ...
 if (active(qq_ini) || (test_output==-1)) {
  ...
  for (s=first_VPA;s<=nsp;s++){
   ...
   for (f=1; f<=n_fleet(s);f++) {
      ...
      //cout<<"Species:"<<s<<" fleet:"<<f<<" dur:"<<duration<<endl;
      if (s>=first_VPA) for (s2_group=1;s2_group<=n_CPUE_s2_group(s,f);s2_group++) {
         ...
         for (a=s2_fa;a<=s2_la;a++) {
              //cout<<"localMaxFleetYear: "<<localMaxFleetYear <<endl;
   ...
   obj_func(s,5)=obj_func(s,5)+sum_penalty;
  }  // species end
 }
 ...
 if (active(SSB_R_alfa) || (test_output==-1)) {
   ...
   for (s=first_VPA;s<=nsp;s++){
     ...
     no=SSB_year_last(s)-SSB_year_first(s)-fa+1;    
      for (y=SSB_year_first(s);y<=SSB_year_last(s)-fa;y++) {       
...
FUNCTION dvariable calc_stom_var(int pred,int spl,int ll)
 ...
 if (stomach_variance==1) {         // log-normal error
   ...
   tmp=log(0.5+sqrt(0.25+(1.0/stom-1.0)/(Stom_var(pred)*Stom_var_fac(pred)*stl_no_samples(spl,ll))));
 }
 else if (stomach_variance==2) {    //normal distributed error
   tmp=Stom_var(pred)*Stom_var_fac(pred)*stom*(1.0-stom)/stl_no_samples(spl,ll);
 }
 else if (stomach_variance==3) {    //structure suitable for Dirichlet
   tmp=Stom_var(pred)*Stom_var_fac(pred)*stl_no_samples(spl,ll)-1;  
   if (tmp<=0) {
...
FUNCTION  evaluate_stomach_contributions
 ...
 for (s=1;s<=nsp;s++) obj_func(s,4)=0.0;
       ...
       for (sd=stl_yq(sq,2);sd<=stl_yq(sq,3);sd++) {
         ...
         for (sp=stl_yqd(sd,2);sp<=stl_yqd(sd,3);sp++) {
          ...
          //cout<<"pred_l:"<<stl_yqdpl(spl,1)<<endl;
           ...
           if (incl_stom(pred,q,stl_yqdpl(spl,1),sy)>=1) { //for the predator and predator length group, include stomach observations in likelihood 0=no inlusion, >=1 include data
             ll=0;
             sum=0.0;
             first_Dirichlet=1;
               ...
               for(prey_l=stl_yqdplp(splp,2);prey_l<=stl_yqdplp(splp,3);prey_l++) {
                 ...
                 if (stl_stom_use_like(spl,ll)==1) {
                   if (stomach_variance==1){ 
                     samp_var=calc_stom_var(pred,spl,ll); 
                     //cout<<"Observed stom: "<<setprecision(5)<<exp(log_stl_stom(spl,ll))<<" Expected stom:"<<stl_E_stom(spl,ll)<<" samp_var:"<<samp_var;
                     ...
                     sum+=log(sqrt(samp_var))+sum_of_squares*0.5/samp_var;
                      ...
                      if (stl_E_stom(spl,ll)<1E-7) {
                        cout<<"Something is wrong (Expected stomach contents<1E-7) !!! Year:"<<stl_y(sy,1)<<" quarter:"<<q<<" pred:"<<pred<<" pred_l:"<<stl_yqdpl(spl,1)<<" Prey:"<<stl_yqdplp(splp,1)<<" Len:"<<prey_l<<endl;
                        cout<<"  min no of samples: "<<min_no_samples(pred)<<" Stom_var_fac: "<<Stom_var_fac(pred)<<" Observed stom: "<<setprecision(5)<<exp(log_stl_stom(spl,ll))<<" Expected stom:"<<stl_E_stom(spl,ll)<<" samp_var:"<<samp_var<<" p:"<<p<<endl;
                        stl_E_stom(spl,ll)=1E-7;
                      }
             ...
             obf+=obj_weight(pred,4)*sum;  //rettet tilbage
             obj_func(pred,4)+=sum;
...
FUNCTION calc_term_F
 ...
 for (s=first_VPA;s<=nsp;s++) for (a=cfa(s);a<=las(s);a++) for (q=fq;q<=lq;q++) if (incl_catch_season_age(s,q,a)>0) {
   sa++; 
   term_F(sa)+=F((lyModel-fy)*lq+q,s,a);
 }
...
FUNCTION calc_term_N
  ...
  // calculate SSB in  year lyModel+1  
  for (s=first_VPA;s<=nsp;s++) {
    SSB(s,lyModel+1)=0.0;
    for (a=faq(q);a<=la_VPA(s);a++) SSB(s,lyModel+1)+=N((lyModel+1-fy)*lq+fq,s,a)*west((lyModel-fy)*lq+fq,s,a)*propmat((lyModel-fy)*lq+fq,s,a);
  }
  // calculate the recruits values  in lyModel + 1  
   //for (s=first_VPA;s<=nsp;s++) N((lyModel+1-fy)*lq+recq,s,fa)=SSB_recruit_last(s);  // recruit  from SSB in the last model year +1 (irrespective of actual year)
   // Move N to sdreport and calc SSB 
   sa=0; 
   ...
   CALC_yq
   for (s=first_VPA;s<=nsp;s++){
      next_SSB(s,lyModel+1)=sum(elem_prod(elem_prod(N(yq,s),west((lyModel-fy)*lq+fq,s)),propmat((lyModel-fy)*lq+fq,s)));         
      for (a=fa+1;a<=la_VPA(s);a++){
        sa++;
        term_N_next(sa)=N(yq,s,a);
        log_term_N_next(sa)= log(term_N_next(sa));
      }
   }  
...
FUNCTION calc_eaten_M2_hist
 ...
 int prey,prey_a,y,q;
 int yq;
 for (y=fy;y<=lyModel;y++) for (prey=first_VPA;prey<=nsp;prey++){
 ...
 //  if (at_age_output[8]) out_predict_raw2("closure",MCMC_iteration,closure);
...
FUNCTION void calc_pred_M2(int yy, int q, d3_array pred_west,d3_array& pred_M1,d3_array& pred_M2, d3_array& pred_F, d3_array& pred_N, d3_array& pred_N_bar_stom);
 int pred, pred_a, prey, prey_a, pred_l, prey_l;
 int y,d;
      ...
      if (pred_N_bar_stom(pred,q,pred_a)>0) { 
        ...
        for (prey=first_VPA;prey<=nsp;prey++) {
         ...
         for (prey=first_VPA;prey<=nsp;prey++) {
           if (pred_prey_comb(d,pred,prey)>0) {
             for (prey_a=faq(q);prey_a<=la(prey);prey_a++) {
               if (size_select_model==1) prey_size=size_sea(yqd,prey,prey_a);
               ...
               if ((pred_size*prey_pred_size_fac(pred))>=prey_size) {
                 ...
                 if (1==1 && part_M2>0) {
                    res2<<yy<<" "<<q<<" "<<pred<<" "<<pred_a<<" "<<prey<<" "<<prey_a<<" "<<part_M2<<endl;
                 }
               }
             }
           }
...
FUNCTION double calc_Yield_from_Fscaling( int s, double F_scaling, dmatrix M, dmatrix M1, dmatrix M2, dmatrix F_sq, dmatrix N_obs, dmatrix weca);
  dmatrix    F(fq,lq,fa,max_a );
  dmatrix    Z(fq,lq,fa,max_a );
  dmatrix    N(fq,lq,fa,max_a );
  int a, q;
  ...
  for (q=fq;q<=lq;q++){
    ...
    for (a=la(s);a>=fa;a--) if (!(a==fa && q<recq)) {
      yield+=N(q,a)*(1.0-exp(-Z(q,a)))/Z(q,a)*F(q,a)*weca(q,a);
...
FUNCTION double find_Fscaling_from_target_yield(int s, double lower, double upper, double target, dmatrix M, dmatrix M1, dmatrix M2, dmatrix pred_F, dmatrix N_obs, dmatrix weca);
  ...
  while ((dif>1E-8) && (iter<100) && (x >=1E-12)) {
    x=(upper+lower)/2;
    y=calc_Yield_from_Fscaling(s,x,M, M1, M2, pred_F, N_obs, weca);
    if (y>=target) upper=x; else lower=x;
    dif=fabs(upper-lower);
    iter++;
  }
  //if (test_output==53) cout<<species_names[s]<<" target yield:"<<setprecision(0)<<y<<"  F scaling:"<<setprecision(3)<<x<<endl; 
...
FUNCTION double SSB_recruit(int s, double SSB, double noise, dmatrix hist_rec_noise,int year);
  double noiseFac, rec;
  ;
  if (noise==0.0) noiseFac=1.0; else 
  {
...
FUNCTION double do_HCR_trigger(int s,double x);  // Estimate F or TAC 
 ...
 //********************************************************************************************* 
FUNCTION void predict_year(int y, int first_sp, int last_sp, int do_M2, int do_catch, int new_year, d3_array& pred_N, d3_array& pred_N_other, d3_array pred_F, d3_array pred_west, d3_array pred_weca, d3_array& pred_C, d3_array pred_M, d3_array pred_M1, d3_array& pred_M2, d3_array& pred_Z);  
 ...
 for (q=fq;q<=lq;q++){
   if (multi==2) {                        // multispecies species mode
     if (do_M2==0) for (s=first_sp;s<=last_sp;s++) pred_Z(s)=pred_F(s)+pred_M1(s)+pred_M2(s);
       ...
       if(use_Nbar==0) {   //use N in the start of the season for calc. of M2
            get_pred_Nbar_stom_at_age(q, pred_N, pred_Nbar, pred_Nbar_stom) ;
            calc_pred_M2(y,q, pred_west, pred_M1, pred_M2, pred_F, pred_N, pred_Nbar_stom);
       }           
       else {  //(use_Nbar==1), //use mean N in the season for calc. of M2. Iterations are needed 
         iter=0; sum2=100;
         for (s=first_VPA;s<=nsp;s++) for(a=fa;a<=la(s);a++)pred_tmp_M2(s,a)=pred_M2(s,q,a);          
 
         while (iter<20 && sum2>1e-4 ) {
           iter++;           
   ...
   if (do_catch) for (s=first_sp;s<=last_sp;s++) 
   {
    for (a=faq(q);a<=la(s);a++) {
      ...
      // Yield
      yield(s,y)+=pred_C(s,q,a)*pred_weca(s,q,a);
      // eaten biomass
...
FUNCTION predict
  ...
  int yq;
  double sumW;
  d3_array    pred_west(1,nsp,fq,lq,fa,max_a);
  d3_array    pred_weca(first_VPA,nsp,fq,lq,fa,max_a);
  d3_array    pred_propmat(first_VPA,nsp,fq,lq,fa,max_a);
  ...
  for (s=first_VPA;s<=nsp;s++) {
    for (q=fq;q<=lq;q++){
     ...
     if (seasonal_annual_catches==1) for (a=fa;a<=la(s);a++) for (q=fq+1;q<=lq;q++) pred_weca(s,q,a)=pred_weca(s,fq,a);    
   }  
  ...
  // repeat for each MCMC iterartion
  ...
  for (MCMC_iteration=1;MCMC_iteration<=no_MCMC_iterations;MCMC_iteration++) {
    ...
    do_predict(pred_west,pred_weca,pred_propmat,MCMC_iteration);
  }


FUNCTION void do_predict(d3_array pred_west, d3_array pred_weca,  d3_array pred_propmat, int MCMC_iteration)
  int s,y,a,q,TAC_year;
  int yq;
  int do_M2, do_catch, add_rec_noise, true_N, new_year;

  double tmp, upper_F,max_F_scale;
  ...
  dmatrix     recruit(first_VPA,nsp,fy,lpy);
  ...
  // truncate noise
  for (s=first_VPA;s<=nsp;s++) if (rec_noise_input(s)==0) {
  ...
  for (s=first_VPA;s<=nsp;s++){
     ...
     for (q=fq;q<=lq;q++){
       ...
       for (a=faq(q);a<=la(s);a++){
        ...
        else if (q>fq) pred_N_assess(s,q,a)=pred_N_assess(s,q-1,a)*exp(-pred_Z_true(s,q-1,a));  
         

        // yield        
        if (q<=lqly) {
          TAC_obs(s,lyModel)+=obs_C(yq,s,a)*weca(yq,s,a); 
         }
        else {
          TAC_obs(s,lyModel)+=pred_F_sq(s,q,a)/pred_Z_true(s,q,a)*pred_N_assess(s,q,a)
                               *(1-exp(-pred_Z_true(s,q,a)))*pred_weca(s,q,a);    
        }
  ...
  if (test_output==53) {
    ...
    cout<<endl<<"Fixed values used in forcast:"<<endl;
   ...
   if (read_expl_pat==1) {
      if (use_read_expl_pat==1) cout<<"input exploitation pattern:";
      else cout<<"F values:";
  ...
  for (y=lyModel+1;y<=lpy;y++){      
    ...
    predict_year(0,first_VPA, nsp, do_M2, do_catch, new_year, pred_N_true,pred_N_other, pred_F_true, 
                   pred_west,pred_weca, pred_C, pred_M, pred_M1, pred_M2_true, pred_Z_true);  
    ...
    if (test_output==53) for (s=first_VPA;s<=nsp;s++) {
       ...
       cout <<"N_true("<<y<<"):"<<setw(10) 
                                << setfixed()<<setprecision(0)<<pred_N_true(s,fq)<<endl;  
    }
    ...
    if ((y==lyModel+1) && (MCMC_iteration==1) ) {
      ...
      sht<<"Species, Quarter, Age,West,Weca,Prop_Mature"<<endl;
      for (s=first_VPA;s<=nsp;s++){
        for (q=fq;q<=lq;q++) {
          for (a=faq(q);a<=la(s);a++){
            sht<<setw(5)<<s<<','<<q<<','<<a<<','<<setfixed()<<setprecision(5)<<setw(10)<<pred_west(s,q,a)<<','<<pred_weca(s,q,a)<<','<<pred_propmat(s,q,a)<<endl;
          }
        }
      }
      sht.close();
     ...
     // Estimate F, using HCR F or HCR TAC from previously year, or from first year input
       if (TAC_F_true(s,y)>-1.0) { //calc F from HCR F
         ...
         if (TAC_cap(s)>0 && (y-lyModel>interYear(s))) {
            ...
            yield1=calc_Yield_from_Fscaling(s,TAC_F_true(s,y)/meanFsq(s),pred_M(s), 
                  pred_M1(s),pred_M2_true(s),pred_F_sq(s),pred_N_true(s),pred_weca(s));
            if (yield1>TAC_cap(s)  ) {
              if (test_output==53) cout<<"Cap TAC adjustment:  ";
              Fscaling=find_Fscaling_from_target_yield(s,real_time_F(s)/meanFsq(s),TAC_F_true(s,y)/meanFsq(s),TAC_cap(s), 
                        pred_M(s), pred_M1(s), pred_M2_true(s),pred_F_sq(s),
                        pred_N_true(s),pred_weca(s));
         ...
         if (y-lyModel<=interYear(s)) {
             TAC_obs(s,y)=calc_Yield_from_Fscaling(s,TAC_F_true(s,y)/meanFsq(s),pred_M(s),
                pred_M1(s),pred_M2_true(s),pred_F_sq(s),pred_N_true(s),pred_weca(s));
        ...
        if (TAC_true(s,y)>0) Fscaling=find_Fscaling_from_target_yield(s,real_time_F(s)/meanFsq(s),upper_F, TAC_true(s,y), pred_M(s), pred_M1(s),
                  pred_M2_true(s), pred_F_sq(s), pred_N_true(s), pred_weca(s));                           
        else Fscaling=0.0;
        ...
        if (test_output==53) {
           if (nsp>1) cout<<species_names[s]<<" "; 
           cout<<"F("<<y<<") is estimated from TAC="<<setprecision(0)<<TAC_true(s,y);
        ...
        cout<<"Error: neither TAC or F have been estimated for year("<<y<<") program stopped"<<endl;
        exit(9);
      } 
      if (test_output==53) {
        if (nsp>1) cout<<species_names[s]<<" ";
   ...
   if (at_age_output[5])             {strcpy(txt,"C"); out_predict_raw3(txt, y, MCMC_iteration,pred_C); }
    ...
    // adjust exploitation pattern if such is specified by input (used for y=years from input
    ...
    for (s=first_VPA;s<=nsp;s++) {
      if (y-lyModel<=interYear(s)) {           // initial condition
        if (TAC_F_true(s,y+1)>-1.0) Mean_F_percieved(s,y+1)= TAC_F_true(s,y+1);
               ...
               find_Fscaling_from_target_yield(s,0.0,max_F_scale, TAC_true(s,y+1), pred_M(s), pred_M1(s),
               pred_M2_true(s), pred_F_sq(s), pred_N_obs(s), pred_weca(s));
      }
      else {
        //cout<< "XXX TEST TAC_obs(s,y+1): "<<TAC_obs(s,y+1)<<endl;
        if (y<lpy) Mean_F_percieved(s,y+1)=meanFsq(s)*find_Fscaling_from_target_yield(s,0.0,max_F_scale, TAC_obs(s,y+1), pred_M(s), pred_M1(s),
                pred_M2_true(s), pred_F_sq(s), pred_N_obs(s), pred_weca(s));
      }
      if (test_output==53) {
         if (y<lpy) {
   ...
   if (test_output==53) cout<<"Start to calc next year(i.e."<<y+1<<") TAC or F from HCR "<<endl;
   ...
   for (s=first_VPA;s<=nsp;s++) if (y+interYear(s)<lpy+1) {
     ...
     if(interYear(s)>1 && TAC_true(s,TAC_year)<0.0 && TAC_F_true(s,TAC_year)<0.0){      
       ...
       // start to find F for intermediate year
        ...
        if (inter_F_TAC(s)==0  && TAC_F_obs(s,y+1)>-1.0) { 
           if (HCR(s)==70) pred_F_obs(s)=pred_F_sq(s)*value(Mean_F_percieved(s,y+1))/meanFsq(s);
           else            pred_F_obs(s)=pred_F_sq(s)*TAC_F_obs(s,y+1)/meanFsq(s);
           
           if (test_output==53) {
              if (HCR(s)==70) cout<<"F is estimated from mean_percieved_F("<<y+1<<"="<<setprecision(3)<<TAC_F_obs(s,y+1)<<endl;
              else cout<<"F is estimated from TAC_F_obs("<<y+1<<")="<<setprecision(3)<<TAC_F_obs(s,y+1)<<endl;
           }
         ...
         //   }
          ...
          Fscaling=find_Fscaling_from_target_yield(s,0.0,max_F_scale, TAC_obs(s,y+1), pred_M(s), pred_M1(s), 
                 pred_M2_true(s), pred_F_sq(s), pred_N_obs(s), pred_weca(s));
        ...
        predict_year(-1,s,s,do_M2,do_catch,new_year, pred_N_obs, pred_N_other, pred_F_obs, pred_west,
                           pred_weca, pred_C, pred_M, pred_M1, pred_M2_true, pred_Z_obs);        
      ...
      switch (HCR(s)) {
       case  1: // constant F, from Fconstant given as input 
        ...
        case 2: // Constant TAC
                ...
                Fscaling=find_Fscaling_from_target_yield(s,0.0,max_F_scale/meanFsq(s),tacMax,pred_M(s),pred_M1(s), 
                          pred_M2_true(s), pred_F_sq(s), pred_N_obs(s), pred_weca(s));
      ...
      case 11: // Estimate TAC from trigger biomass in the TAC year
                ...
                Fscaling=find_Fscaling_from_target_yield(s,Fmin/meanFsq(s),max_F_scale/meanFsq(s),tacMax,
    ...
    case  47: // SPECIAL CASE, North Sea,and Kattegat Cod
      ...
      case  70: // Commission Policy evaluation
                //
                // input: Constant.F=FMSY, T1=Fpa, T2=Bpa,
                if (test_output==53) {
                   cout<<"Obs F("<<TAC_year-1<<")="<<setprecision(3)<< Mean_F_percieved(s,TAC_year-1)<<
                     " FMSY="<<constantF(s)<<"  Fpa="<<T1(s)<<endl;
                  cout<<"SSB("<<TAC_year<<")="<<setprecision(0)<<SSB_obs(s,TAC_year)<<"  Bpa="<<T2(s)<<endl;
                }
                if (Mean_F_percieved(s,TAC_year-1)<=constantF(s) && SSB_obs(s,TAC_year)>T2(s)) {
                  sub_HCR(s)=1;
                  Fmax=constantF(s);
                }
                else if (Mean_F_percieved(s,TAC_year-1)<=T1(s) && SSB_obs(s,TAC_year)>T2(s)) {
                  sub_HCR(s)=2;
                  if (constantF(s)>Mean_F_percieved(s,TAC_year-1)) Fmax=constantF(s);
                  else Fmax=value(Mean_F_percieved(s,TAC_year-1));
                }
                else {
                  sub_HCR(s)=3;
                  // Estimate F from a target SSB in the TAC year+1 and max F=Fpa
                  Fscaling=find_Fscaling_from_target_SSB(s,1,0.0,T1(s)/meanFsq(s),T2(s),pred_M(s),
                      pred_M1(s),pred_M2_true(s),pred_F_sq(s),pred_N_obs(s),pred_west(s),pred_propmat(s),y);
                  if (Fscaling>0) Fmax=meanFsq(s)*Fscaling; else Fmax=0.0;
                  if (Fmax<real_time_F(s)) Fmax= real_time_F(s);
                  
                }
                if (test_output==53) cout<<"HCR=70 sub model:"<<sub_HCR(s)<<"  F("<<TAC_year<<")="<<setprecision(3)<<Fmax<<endl;
                closure(s,TAC_year)=sub_HCR(s);
                break;
       ...
       case 99: // (both 98 and 99) & closure if the fishery in second quarter
             ...
             if (HCR(s)==98) {
                ...
                Fscaling=find_Fscaling_from_target_yield(s,Fmin/meanFsq_tmp,max_F_scale/meanFsq_tmp,tacMax,
      ...
      case 333: // SPECIAL CASE, Blue whiting, 2005 management plan
                ...
                if (BW_init_phase) {
                  ...
                  Fscaling=find_Fscaling_from_target_yield(s,Fmin/meanFsq(s),max_F_scale/meanFsq(s),yield1,
                               pred_M(s),pred_M1(s), pred_M2_true(s), pred_F_sq(s),
                               pred_N_obs(s), pred_weca(s));
      ...
      if (Fmax>0) yield1=calc_Yield_from_Fscaling(s,Fmax/meanFsq(s),pred_M(s),pred_M1(s),pred_M2_true(s),pred_F_sq(s),
                    pred_N_obs(s),pred_weca(s));
      else yield1=0.0;
      ...
      if (F_constraint(s,1)!=0 || F_constraint(s,2)!=0) {
         ...
         yield1=calc_Yield_from_Fscaling(s,Fmax/meanFsq(s),pred_M(s),pred_M1(s),pred_M2_true(s),
                 pred_F_sq(s),pred_N_obs(s),pred_weca(s));
      ...
      // Yield constraints   
      if (0!=TAC_constraint(s,1) || 0!=TAC_constraint(s,2)) { 
        ...
        if (change<TAC_constraint(s,1)) { // decrease in yield is too much raise F
            Fscaling=find_Fscaling_from_target_yield(s,Fmax/meanFsq(s), max_F_scale/meanFsq(s), 
              ...
              Fscaling=find_Fscaling_from_target_yield(s, Fmin/meanFsq(s),max_F_scale/meanFsq(s),
       ...
       // Commission Policy evaluation
       if (HCR(s)==70){
          if (Fscaling >=0 && sub_HCR(s)==3) {   //Constraints have been in use and stock outside safe biological limits
             if (test_output==53) cout<<"Constraints in use. F("<<TAC_year<<")="<<Fmax<<endl;
             if (Fmax>Mean_F_percieved(s,TAC_year-1)) {  //F is not allowed to increase
               Fmax=value(Mean_F_percieved(s,TAC_year-1));
               constraints(s,y)+8;
               if (test_output==53) cout<<" F("<<TAC_year<<") decreased to "<<Fmax<<endl;
             }
             tmp=calc_SSB_from_Fscaling(s,Fmax/meanFsq(s),1,pred_M(s),pred_M1(s),pred_M2_true(s),pred_F_sq(s),
                pred_N_obs(s),pred_N_obs(s,recq,fa),pred_west(s),pred_propmat(s));
                
             if (test_output==53) cout<<"SSB("<<TAC_year+1<<")="<<setprecision(0)<<tmp
                 <<"   SSB_obs("<<TAC_year<<")="<<SSB_obs(s,TAC_year)<<endl;

             if (tmp < SSB_obs(s,TAC_year)) { //SSB is not allowed to decrease
                 constraints(s,y)+16;
                 if (test_output==53) cout<<"SSB("<<TAC_year+1<<") < SSB_obs("<<TAC_year<<")"<<endl;
                 tmp=find_Fscaling_from_target_SSB(s,1,0.0,max_F_scale/meanFsq(s),SSB_obs(s,TAC_year),pred_M(s),
                   pred_M1(s),pred_M2_true(s),pred_F_sq(s),pred_N_obs(s),pred_west(s),pred_propmat(s),y);
                 if (Fscaling>0) Fmax=meanFsq(s)*Fscaling; else Fmax=0.0;
                 if (Fmax<real_time_F(s)) Fmax= real_time_F(s);
      ...
      if (HCR_F_TAC(s)==1) {       //implement HCR using estimated TAC to estimate true F
        ...
        if (HCR(s)==24 && F_cap(s)>0) { 
           ...
           if (yield1>0) Fscaling=find_Fscaling_from_target_yield(s,real_time_F(s)/meanFsq(s),upper_F, yield1, pred_M(s), pred_M1(s),
                    pred_M2_true(s), pred_F_sq(s), pred_N_true(s), pred_weca(s));                           
           else Fscaling=0.0; 
           if (test_output==53) cout<<"Fscaling: "<<setprecision(3)<< Fscaling<< "  F:"<< Fscaling*meanFsq(s)<<endl;          
           if (Fscaling*meanFsq(s)>F_cap(s)) yield1=calc_Yield_from_Fscaling(s,Fscaling*meanFsq(s),pred_M(s),pred_M1(s),pred_M2_true(s),
                 pred_F_sq(s),pred_N_true(s),pred_weca(s));
    ...
    for (s=first_VPA;s<=nsp;s++){
     ...
     for (a=fa;a<=la(s);a++) {
       ...
       for (y=fy;y<=lyModel;y++){
         lqLocal=(y==lyModel)?lqly:lq;
         no++;
         for (q=fq;q<=lqLocal;q++) if (!((a==fa) && (q <recq)))tmp+=M2((y-fy)*lq+q,s,a);
 ...
 for (s=first_VPA;s<=nsp;s++){
   for (f=1; f<=n_fleet(s);f++) {
      ...
      for (s2_group=1;s2_group<=n_CPUE_s2_group(s,f);s2_group++) {
         ...
         for (a=s2_fa;a<=s2_la;a++)
         if (qq_s2(sp_fl,s2_group)>0 && qq_s2(sp_fl,s2_group)<=mins && s>=first_VPA) {
...
FUNCTION check_print_par
 int s;
   ...
   cout << setw(12) << setprecision(5) << setfixed();
    ...
    cout << endl << "F_q_ini"<<endl<<F_q_ini<<endl;
...
FUNCTION print_input_par
  int s,fl;
  cout << endl;
  cout << "First year, last year (data):              " << fy<<" " <<lyData <<endl;
  cout << "First year, last year (model):             " << fy<<" " <<lyModel <<endl;
...
FUNCTION void print_summary()
 ...
 res <<"Year Quarter Species.n Age M1 M2 M F Z N N.bar C.hat C.obs west weca Yield propmat BIO SSB Lsea ration"<<endl;
  for (s=first_VPA;s<=nsp;s++){
    for (y=fy;y<=lyModel;y++){
      lqLocal=(y==lyModel)?lqly:lq; 
      for (q=fq;q<=lqLocal;q++){
...
FUNCTION void print_summary_table()
 ...
 for (s=first_VPA;s<=nsp;s++){
    ...
    res<<"Year         Recruits          SSB            TSB            SOP           mean-F"<<endl;
    ...
    if (fq==lq || lqly==lq) {  // annual assessment, calch SSB the year after the last assess. year
      ...
      CALC_yq
      for (a=fa+1;a<=la(s);a++){
         tmp+=N((lyModel+1-fy)*lq+fq,s,a)*west(yq,s,a)*propmat(yq,s,a)*exp(-(prop_M(s)*M(yq,s,a)+prop_F(s)*F(yq,s,a))) ;
      }
      res<<lyModel+1<<"  ";
 ...
 res2<<"Species.n Year  Rec SSB TSB SOP SOP.hat mean.F Eaten"<<endl;
  ...
  for (s=first_VPA;s<=nsp;s++){
    for (y=fy;y<=lyModel;y++){
      ...
      for (q=fq;q<=lqLocal;q++){
        CALC_yq
        for (a=cfa(s);a<=la(s);a++)  if (!(a==fa && q<recq)) {
         yield_hat(s,y)+=weca(yq,s,a)*C_hat(yq,s,a);
        }
      }
    }
  }
 ...
 for (s=first_VPA;s<=nsp;s++){
    for (y=fy;y<=lyModel;y++){
      ...
      res2<<s<<" "<<y<<"  "<<N(yq,s,fa)<<" "<<SSB(s,y)<<" "<<
            TSB(s,y)<<" "<<yield(s,y)<<" "<<yield_hat(s,y)<<" "<<Mean_F(s,y)<<" "<<eaten_M2(s,y)<<endl;
    }
    ...
    if (fq==lq || lqly==lq) {  // annual assessment, calch SSB the year after the last assess. year
     ...
     res2<<s<<" "<<lyModel+1<<"  NA "<<tmp<<" NA NA NA NA NA "<< endl;
    }
 }
 res<<endl;
 res2.close();
...
FUNCTION void print_vulnerab()
 ...
 res <<"Pred.no Prey.no vulnerability"<<endl;
 i=0;   
 for (pred=1;pred<=npr;pred++){  
      for (prey=first_VPA;prey<=nsp;prey++) {
        if (pred_prey_comb(d,pred,prey)>0) {
...
FUNCTION void print_summary_stom()
 int y,q,ll,pred,prey,pred_l,prey_l,pred_l_class,size_model;
 dvariable stomcon,stomcon_hat,obj_con,avail_food_,N_l;
 dvariable prey_l_class,prey_length_mean,pred_l_mean,suitab,residual,prey_avail,prey_avail_part,samp_var,N_samples,sum_p,like_dir,p; 
 int sy,sq,sp,spl,splp;  //stomach index counters
 int observed=1; 
 double pred_w,prey_w,pred_siz,prey_siz,N_haul,stom_used_all;
 ...
 res<<"Predator.length.mean Predator.weight Predator.size Predator.available.food Diri.p Diri.sum.p Diri.like Prey.no  ";
 res<<"Prey.length.class Prey.length.mean Prey.weight Prey.size N.haul N.samples stomcon  stomcon.hat ";
 res<<"Residual L.N.bar Prey.avail Prey.avail.part Suit Stom.var obj.contrib stom.input stom.used.all stom.used.like stom.used.avail stom.used.prey ";
 res<<endl;
     ...
     for (sd=stl_yq(sq,2);sd<=stl_yq(sq,3);sd++) { 
      ...
      for (sp=stl_yqd(sd,2);sp<=stl_yqd(sd,3);sp++) {
       ...
       for (spl=stl_yqdp(sp,2);spl<=stl_yqdp(sp,3);spl++) {
         ...
         if (incl_stom(pred,q,stl_yqdpl(spl,1),sy)>=1) stom_used_all=1; else stom_used_all=0;
         ...
         for (splp=stl_yqdpl(spl,2);splp<=stl_yqdpl(spl,3);splp++) {
           ...
           for(prey_l=stl_yqdplp(splp,2);prey_l<=stl_yqdplp(splp,3);prey_l++) {
             ...
             if (stl_stom_use_like(spl,ll)==1) {
               ...
               samp_var=calc_stom_var(pred,spl,ll);   // Calculated sample variance
               if (stomach_variance==3) {
                  samp_var= stl_E_stom(spl,ll) * (1.0-stl_E_stom(spl,ll))/(sum_p+1);
                  p=sum_p*stl_E_stom(spl,ll);
               }  else p=0;

               if(stomach_variance!=3 ){   // normal or log-normal distribution or test
                 obj_con=log(sqrt(samp_var))+square(stl_stom(spl,ll)-stl_E_stom(spl,ll))*0.5/samp_var;
               } else obj_con=0;            
             ...
             res<<pred_w<<" "<<pred_siz<<" "<<avail_food_<<" "<<p<<" "<<sum_p<<" "<<like_dir<<" "<<prey<<" "<<prey_l_class<<" "<<prey_length_mean<<" ";
             res<<prey_w<<" "<<prey_siz<<" "<<N_haul<<" "<<N_samples<<" "<<stomcon<<" "<<stomcon_hat<<" ";
             res<<residual<<" "<<N_l<<" "<<prey_avail<<" "<<prey_avail_part<<" "<<suitab<<" "<<samp_var<<" "<<obj_con<<" ";
             res<<stl_stom_input(spl,ll)<<" "<<stom_used_all<<" "<<stl_stom_use_like(spl,ll)<<" "<<stl_stom_use_avail(spl,ll)<<" "<<stl_stom_use_prey(spl,ll)<<endl;
            }  
          }
        }
       }
...
FUNCTION void print_catch_residuals()
 ...
 if (seasonal_annual_catches==0) for (s=first_VPA;s<=nsp;s++){
   for (q=fq;q<=lq;q++){
 ...
 if (seasonal_annual_catches==1) for (s=first_VPA;s<=nsp;s++){
     res2 << endl<<"# Species,Quarter,Year";
     for (a=fa;a<=la(s);a++) res2<<", Age_"<<a;
     for (y=fy;y<=lyModel;y++){
        res2 << endl << s<< ",9," <<y ;
...
FUNCTION void print_catch_survey_residuals()
 ...
 fleet=-9;
 res <<"data Species.n Quarter Year fleet Age observed model residual"<<endl;
   ...
   for (q=fq;q<=lq;q++){
     for (y=fy;y<=lyModel;y++){
        ...
        for (a=faq(q);a<=la(s);a++) {
         ...
         res <<' '<<setscientific()<<exp(log_obs_C(yq,s,a)) <<' '<<C_hat(yq,s,a)<<' '<< log_obs_C(yq,s,a)-log(C_hat(yq,s,a))<<endl;
         else res <<" -99.9 -99.9 -99.9 "<<endl;
       }
     }
   }
 }
   ...
   for (y=fy;y<=lyModel;y++){
      for (a=fa;a<=la(s);a++) {
       res << setfixed()<<"catch " << s<< " " <<" 9 " <<y <<' '<<fleet<<' '<<a<<' ';
        if ((a>=cfa(s)) && (y<=lyModel) && (incl_catch_season_age(s,1,a)==1) && (C_hat_annual(y,s,a)>0))
          res <<' '<<setscientific()<<exp(log_obs_C_annual(y,s,a)) <<' '<<C_hat_annual(y,s,a)<<' '<< log_obs_C_annual(y,s,a)-log(C_hat_annual(y,s,a))<<endl;
       else res <<" -99.9 -99.9 -99.9 "<<endl;;
     }
   }
 }
...
FUNCTION void print_size_pref();
 int s;
  ...
  sf<<"species.n size.model size.ratio size.var"<<endl;
  for (s=1;s<=npr;s++) {
    sf <<s<<"  "<<size_selection(s)<<"  "<<pref_size_ratio(s)<<"  "<<var_size_ratio(s)<<endl;
  }
  sf.close();

FUNCTION void print_min_max_size_pref();
  ofstream sf("min_max_size_pref.out",ios::out);
  sf<<min_pred_prey_size_ratio<<endl<<max_pred_prey_size_ratio;
  sf.close();
...
FUNCTION void print_vars_ICES(adstring txt,adstring intype)
 ...
 max_g=(lyModel-fy)/yl;     // no of sub-tables
 if ((lyModel-fy)>(max_g*yl)) max_g++; 
 ...
 for (s=first_VPA;s<=nsp;s++){
   ...
   for (g=1;g<=max_g;g++) {
     ...
     for (y=fy+(g-1)*yl;y<min(fy+g*yl,lyModel+1);y++) det <<setw(wi)<< y;
     det << endl; 

     for (a=fa;a<=la(s);a++){
       det << setw(wi) << a;
       for (y=fy+(g-1)*yl;y<min(fy+g*yl,lyModel+1);y++) {
         tmp=0.0;
         if (strcmp(intype,"obs_C") == 0) {
            for(q=fq;q<=lq;q++) if (!(a==fa && q<recq)) tmp+=obs_C((y-fy)*lq+q,s,a);
            ...
            for(q=fq;q<=lq;q++) if (!(a==fa && q<recq)) tmp+=F((y-fy)*lq+q,s,a);
            det << setw(wi) << setprecision(4) << setfixed()<< tmp;
         }
         else det <<"ERROR"<<endl;
       }
     ...
     if (strcmp(intype,"N") == 0) { 
        det <<endl<<"TSB";
        for (x=1;x<=wi-3;x++) det << " ";
        for (y=fy+(g-1)*yl;y<min(fy+g*yl,lyModel+1);y++) det<<setw(wi)<<setprecision(0)<<setfixed()<<TSB(s,y);
        det <<endl<<"SSB";
        for (x=1;x<=wi-3;x++) det << " ";
        for (y=fy+(g-1)*yl;y<min(fy+g*yl,lyModel+1);y++) det<<setw(wi)<<setprecision(0)<<setfixed()<<SSB(s,y);
     }
     else if (strcmp(intype,"F") == 0) {
       ...
       for (y=fy+(g-1)*yl;y<min(fy+g*yl,lyModel+1);y++) det<<setw(wi)<<setprecision(0)<<setfixed()<<yield(s,y);
     }
...
FUNCTION void print_SSB_R()
 ...
 SR <<"Species.n model alfa  beta std"<<endl;
 for (s=first_VPA;s<=nsp;s++){
   SR << s << " " <<SSB_Rec_model(s)<<" ";
   if (SSB_Rec_model(s)<=2)      SR<<SSB_R_alfa(s)<<" " <<SSB_R_beta(s)/SSB_R_beta_cor(s);
...
FUNCTION void  print_objective_func_file()
 int s;
  ...
  obj<<"Species.n catch CPUE SSB.Rec  stomachs penalty all n.catch n.CPUE n.SSB.R n.stom n.all.obs n.par"<<endl;
  for (s=1;s<=nsp;s++)  obj<<s<<"  "<<obj_func(s)<<" "<<obf<<" "<<no_obj_obs(s)<<" "<<initial_params::nvarcalc()<<endl;
  obj.close();
   
   
FUNCTION void print_survey_data_noise(int no_set)
 int s, y, q, a, f, s2_fa, s2_la, s2_group, no, sp_fl,localMaxFleetYear;
  dvariable N_survey,duration;
  dmatrix    N_survey_out(fy,lyModel+1,fa,max_a);       // survey and noise
  char txt[40];
  char txt2[10];
...
FUNCTION void print_catch_data_noise(int no_set)
  d3_array   C_hat_noise(fy,lyModel,fq,lq,fa,max_a);       // catch and noise
   ...
   ofstream can(txt,ios::out);
    ...
    for (s=first_VPA;s<=nsp;s++){
     ...
     for (s2_group=1;s2_group<=n_catch_s2_group(s);s2_group++) {
           ...
           if (seasonal_annual_catches==0) {
             ...
             for (a=s2_fa;a<=s2_la;a++) if (incl_catch_season_age(s,q,a)>0) {
               ...
               if (q<=lqLocal) {
                 ...
                 C_hat_noise(y,q,a)=value(C_hat(yq,s,a))*exp(randn(rnd)* sqrt(value(catch_s2(s,i,s2_group))));  //log norm dist noise
               } //lqLocal
             }  //a-loop
           } // Seasonal catches                       =
         }  // q-loop
       ...
       for (y=fy;y<=lyModel;y++) can<<"# sp:"<<s<<" "<<y<<endl<<C_hat_noise(y)<<endl;
    } // species loop
    can.close();
  } 
...
REPORT_SECTION
 ...
 dvar_matrix out2(fy,lyModel,first_VPA,nsp);
 dvar_vector out3(1,6);
 ...
 report << "unweighted objective function contributions (total): " <<endl
    <<"                Catch    CPUE    S/R    Stom.  Penalty     Sum"<< endl;
  for (s=1;s<=nsp;s++) {
  tmp=0.0;
    ...
    for (i=1;i<=5;i++) {
       ...
       if (i<5) report<< setw(8) << setfixed()<<setprecision(1)<<obj_func(s,i);
       else  report<< setw(10) << setscientific()<<setprecision(2)<<obj_func(s,i);
    }
    out3(6)=out3(6)+tmp;
   report<< setw(8) << setfixed()<<setprecision(1)<<tmp << endl;
 }
  if (nsp>1) report << "Sum         ";
 if (nsp>1) for (i=1;i<=6;i++) {
   if (i != 5) report<< setw(8) << setfixed()<<setprecision(1)<<out3(i);
   else  report<< setw(10) << setscientific()<<setprecision(2)<<out3(i);
 }
 
 //for (s=1;s<=nsp;s++) no_obj_obs(s,3)=lyModel-fy+1;
 ...
 if (all_do_effort==0) {
   ...
   report<<endl;
   for (y=fy;y<=lyModel;y++) report<<y<<':'<< setw(8) << setprecision(3) << setfixed()<<out2(y)<<endl;
 }
 // if (use_creep(1)>0){ 
 ...
 report << endl << "-----------------";
 if (seasonal_annual_catches==1) report<<"\nPlease note: Season effects are copied from input file"<<endl;
 for (s=first_VPA;s<=nsp;s++) {
   report << endl;
 ...
 report<<            "----------"<<endl<<"         ";
 for (s=first_VPA;s<=nsp;s++)for (y=fy;y<=lyModel;y++) out2(y,s)=Mean_F(s,y); 
 ...
 if (multi>=1) {
   for ( y=fy;y<=lyModel;y++) for (q=fq;q<=lq;q++) calc_M2(y,q);
 ...
 for (s=first_VPA;s<=nsp;s++){
   ...
   if (SSB_Rec_model(s)<=2)  report<<setw(12) << setprecision(3)<< setfixed()<<SSB_R_alfa(s) <<"   " <<setscientific()<<SSB_R_beta(s)/SSB_R_beta_cor(s);
  ...
  if (multi>=1) {
     ...
     report<<"---------------------------";
     for (d=1;d<=no_areas;d++) {
       if (no_areas>1) report<<endl<<"Area: "<<d<<endl; else report<<endl;
       report<<"           Other-food     ";
     ...
     if  (sum(size_selection)==0) {
       report<<"No size selection has been chosen for all predators"<<endl;                 
        ...
        else if (size_selection(pred)==11) report<<"log-norm. sum ";
     ...
     report<<endl<<endl<<"Stomach variance:    value    internal "<<endl;
     for (pred=1;pred<=npr;pred++) report <<species_names(pred)<<setw(15)<<setprecision(3)<<setfixed()<<Stom_var(pred)*Stom_var_fac(pred)<<"    "<<Stom_var(pred)<<endl;;
     
     if (phase_season_overlap>0) {
       report<<endl<<endl<<"Predator prey season overlap"<<endl;
       report<<            "----------------------------"<<endl;
       for(d=1;d<=no_areas;d++) {
         if (no_areas>1) report<<endl<<"Area: "<<d<<endl;
         for (pred=1;pred<=npr;pred++) {
 ...
 if (do_prediction_mean!=1) { //retrospecive analysis
  if (multi!=1) {
   ...
   if (multi>1) {
      print_part_M2();
      if (test_output>=1) cout <<"Output file Part_M2.rep is done"<<endl;
   ...
   print_F_status_quo();
   if (test_output>4) cout <<"Output file print_F_status_quo.out is done"<<endl;
  ...
  if (test_output<0) {
     print_catch_data_noise(-test_output);
     print_survey_data_noise(-test_output);
  }
