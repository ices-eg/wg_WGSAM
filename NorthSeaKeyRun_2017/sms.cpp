...
v_last_sp=nsp;
 ...
 yq2=(lyData-fyData+2)*lq+lq;
  vAll2_fa2.allocate(1,yq2,1,nsp);
 for (y=fyData;y<=lyData+2;y++) for (q=fq;q<=lq;q++) {
  i=(y-fyData)*lq+q;
  vAll2_fa2(i)=faq(q);
 }
 ...
 for (i=1;i<=yq2;i++) for (s=first_VPA;s<=nsp;s++) vVPA_la2(i,s)=la(s);
  vAll2_la2.allocate(1,yq2,1,nsp);
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
 ...
 if (phase_SSB_R_alfa>0 && est_calc_sigma(3)==0)  for (s=first_VPA;s<=nsp;s++) {parNo++; parexp<<"SSB_R_s2_ini "<<parNo<<" "<<s<<" -1 -1 -1 -1 -1 -1 -1"<<endl; }
  ...
  Z.allocate(1,yq1,1,v_last_sp1,vAll_fa1,vAll_la1,"Z");
    ...
    Z.initialize();
  #endif
  N.allocate(1,yq2,1,v_last_sp2,vAll2_fa2,vAll2_la2,"N");
  #ifndef NO_AD_INITIALIZE
    N.initialize();
  #endif
  ...
  if (mceval==0) {
   ...
   if (mceval==0) {
    ...
    for (pred=1;pred<=npr;pred++) {
      cout <<endl<<species_names[pred];
      for (prey=first_VPA;prey<=nsp;prey++) {     
        if ((max_pred_prey_size_ratio(pred,prey)>0 && min_pred_prey_size_ratio(pred,prey)==1000) ||
            (max_pred_prey_size_ratio(pred,prey)<0 && min_pred_prey_size_ratio(pred,prey)!=1000))
 ...
 int s;
   cout << setw(12) << setprecision(5) << setfixed();
    ...
    cout << endl << "qq_ini"<<endl<<qq_ini<<endl;
     cout << endl << "qq_s2_ini"<<endl<<qq_s2_ini<<endl;
     cout << endl << "SSB_R_alfa"<<endl<<SSB_R_alfa<<endl;
    cout << endl << "SSB_R_beta_ini"<<endl<<SSB_R_beta_ini<<endl;
    if (active(vulnera) && multi>=1) {
        ...
        cout <<"init_stl_other_suit_slope: "<<endl<<init_stl_other_suit_slope<<endl;
