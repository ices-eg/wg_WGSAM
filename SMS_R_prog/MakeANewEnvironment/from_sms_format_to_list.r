From_SMS_format_to_list<-function(otherPredExist=T,catchMultiplier=1,code.name=c('COD','WHG'),exchangeDir=file.path(root,exchangeDir),addfn='a') {
...
if (otherPredExist) {
  ...
  length(WSEA)
  N<-scan(file.path(data.path,'other_pred_N.in'),comment.char='#')
