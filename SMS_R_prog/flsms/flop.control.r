...
###
write.FLOPtrigger.control<-function(control,file="op_trigger.dat",path=NULL,nice=TRUE,writeSpNames=T) {
  ...
  "stochastic.recruitment" ={if (nice) {
      ...
      "F.or.C"              ={if (nice) {
                                ...
                                cat("# Update N by:\n",
                                    ...
                                    "#    31= as 1 but get values from file op_mulTargetf.in\n",file=file,append=T,sep="")
                                    wr.matrix.nice(slot(control,x),VPA.species)
                                } else wr.matrix(slot(control,x),x)
                             },
...
read.FLOPtrigger.control<-function(file="op_trigger.dat",n.VPA,n.other.pred) {
...
read.FLOP.control<-function(file="op.dat",n.VPA,n.other.pred,n.pred) {
  ...
  for (x in n.) {
    switch(x,
    "species.names"            = { species.names<-readLines("species_names.in", n=n.sp)
                                   species.names<-gsub('_',' ',species.names)
                                   species.names<-sub('[[:space:]]+$', '', species.names)
                                   slot(control,x)<-species.names; },
     "years.wsea"              = {slot(control,x)<-matrix(opt[n:(n-1+2*n.sp)],ncol=n.sp,nrow=2,
                                   dimnames=list(c("first","last"),species.names),byrow=TRUE); n<-n+2*n.sp},
