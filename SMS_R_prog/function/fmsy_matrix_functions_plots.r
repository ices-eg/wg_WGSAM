...
plot.matrix<-function(tit='Yield',type=c('Yield','SSB','Recruit','Fbar')[1],first.sp='COD',spNames=sp.names[17:24],my.dev='png',out.file='a',add.to.file.name='',plot.ref=F,scenario.dir='a',a=a) {
  ...
  dummy<-by(a,list(a$Species.n),function(x) {
    ...
    ref.lines<-function(sp){
       ...
       abline(h=ref[sp,'Bpa']/1000,col='blue',lwd=2)
       abline(h=ref[sp,'Blim']/1000,col='red',lwd=2)
    }
    ii<-1
    for (i in SP.codes) {
