...
OP.simulate<-function(
          ...
          singleSp=F,      # Mak single species plot      
  ...
  if ((do.plot.condense | ReduceTable ) & n.run >1) {
    ...
    if (my.area=='North Sea') {
        ...
        if (singleSp) {
          ...
          print(xyplot(recruit/1000000~SSB/1000 |Species,data=a,
        ...
        do.plot.condense<-!singleSp  # do not make the ususl plots if single species
        ...
        if (do.plot.condense) {
          spNames<-sp.names[(nsp-nsp.VPA+1):(nsp-2)]  # all vpa species except PLE and Sole
