...
HPC<-TRUE   # use High Performance Computer (on Unix)
if (!HPC) memory.limit(size = 85000)
...
batch<-T # with batch==T several runs are made with condensed output, while batch==F makes one run with detalied output
...
if (stochastic.recruitment==0) { 
  my.last.year<-2030; years.in.average<-2;   no.iters<-1 
} else {
...
recruit.adjust.factor[3]<-0.75  # Haddock, special case
