...
do.plot<-function(){
 ...
 for (s in (1:length(all.pred.col))) all.names[s]<-new.names[all.pred.col[s]]
  ...
  palette(c('red','green','plum','blue','cyan','yellow','coral','skyblue','purple','magenta','limegreen','pink' ))
  # palette("default")
 ...
 by(a,list(a$Prey),function(x) {
  ...
  by(x,list(x$Prey.age,x$Prey),function(x) {
      ...
      if ((i %% (nox*noy-1))==0) {
        ...
        if (makeAllGraphs) filename=file.path(PartialM2.dir,filename)
...
do.plot.q<-function(){
  ...
  #subset(s,is.na(Part.M2))
  s<- subset(s,!is.na(Part.M2))
  a<-aggregate(s$Part.M2,list(s$new,s$Year,s$Quarter,s$new.no,s$Prey,s$Prey.no,s$Prey.age),sum)
  ...
  by(a,list(a$Prey,a$Quarter),function(x) {
    ...
    by(x,list(x$Prey.age,x$Prey),function(x) {
      ...
      if ((i %% (nox*noy-1))==0) {
          #newplot(dev,nox,noy,w8=12,w11=16,Portrait=F,pointsize=12);
        if (dev =='png') poi<-20 else poi<-12
        filename<-paste0("Partial_M2_Q",x[1,'Quarter'],'_',x[1,'Prey'])
        if (makeAllGraphs) filename=file.path(PartialM2.dir,filename)
