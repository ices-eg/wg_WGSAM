...
dirs<-  c("NS_paper_single","NS_paper_uniform","NS_paper_uniform_confined","NS_paper_size","NS_paper_size_fixed","NS_paper_size_mesh")       # directories with output to compare
labels<-c("0) Single ss","1) Uniform","2) Uniform, confined","3) log-normal","4) log-normal, fixed parameters","5) log-normal, mesh selection")  # labes for each scenario (directory with data)
...
dirs<-  c("NS_paper_single","NS_paper_uniform_confined")       # directories with output to compare
labels<-c("0) Single ss","2) Uniform, confined",)  # labes for each scenario (directory with data)
...
bb<-subset(aa,name=='hist_SSB')
#bb<-subset(aa,name=='avg_F')
bb<-subset(aa,name=='rec_sd')
