...
if (do.simulation) {
  ...
  SMS.files.single<-c("natmor.in","canum.in","west.in","weca.in","propmat.in","fleet_catch.in",
                      ...
                      "SSB_R.in","Prediction_F.in","reference_points.in","predict_stock_N.in","proportion_M_and_F_before_spawning.in",
                      "Exploitation_pattern.in","HCR_options.dat","SMS.dat","sms.exe")
  ...
  for (option in (iterations)) {
      ...
      #}
       no.MCMC.iterations <-n.iterations
       HCR@last.prediction.year <- last.year.in.mean
       
       smallF<-0.001
      if (scenario=="Ftarget") {
            HCR@constant.F[1]<-option
            HCR@HCR[1]<-1
      }
      ...
      #run SMS
      shell(paste(file.path(data.path,scenario,"sms.exe"),"-mceval",sep=" "), invisible = TRUE)
