...
for (dir in dirs) {
   ...
   rat<-data.frame(scen=labels[which(dirs==dir)],vari='ration',a)
   names(rat)<-c("scenario","Variable","Species","Year","Age","Value")

    

 if (dir==dirs[1]) all<-rbind(rat) else all<-rbind(all,rat)
}
