make.OP.dat<-function(my.area='North Sea',my.last.year=2020,first.year.output=2020,do.indicators=F,stochastic.recruitment=0,recruit.adjust.CV=0, recruit.adjust.factor=1,      y.end=2019,y.first=2017,y.first.other=2019) {
  ...
  OP<-FLOP.control(
      ...
      OP@years.other['first-year',] <-y.first.other;
...
#          OP@rec.noise['upper',]<-c(   2.0,     2.0,     2.0,      2.5,       2,       2,         2,     2.5,      2.5,    2)
 ...
 #         OP@rec.noise['upper',]<-c(   2.0,     2.0,     2.0,      2.5,       2,       2,     2 ,   2,     2.5,      2.5,    2)
      ...
      if (my.area=='Baltic Sea') {
                                     #Cod    Herring   Sprat 
          OP@rec.noise['lower',]<-c(            -2.5,    -1.5 )
          OP@rec.noise['upper',]<-c(             2.0,     2.0 )
