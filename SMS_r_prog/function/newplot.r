...
newplot<-function(dev,nox=1,noy=1,Portrait=FALSE,filename='a',dir=data.path,w8=8,w11=11,pointsize=12,doGgplot=FALSE)
  {
    noxy<<-nox*noy
    ...
    if (dev=="ps") {
       ...
       outfile <- file.path(dir,filename.out)
       if (doGgplot){
         if (Portrait==TRUE) ggsave(filename=paste(outfile),width =200*w8,height =200*w11,units = "px" ,pointsize = pointsize,bg = "white") else ggsave(filename=paste(outfile),width =200*w11,height =200*w8,units = "px" ,pointsize = pointsize,bg = "white")
       } else {
