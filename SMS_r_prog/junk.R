...
by(b,b$pp,function(x)summary(x$box.res))

b$Length<-b$Predator.length/10
b$yq<-paste(b$Year,b$Quarter)
b$pp<-paste(b$Predator,b$Prey,sep=' eating ')
X11()
ggplot(ab, aes(x =factor(Length), y = box.res)) + 
...
my.pred<-c("Cod")
my.col<-"red"
by(b,list(b$Predator,b$Prey,b$Area),function(x) {
  print(summary(x$Residual))
