...
SMS.stocks<-lapply(SMS.stocks,function(x){ window(x, start = 1963, end = 2010)} )
...
a<- FLStock(name =SMS.stocks[[sp]]@name, desc = "test",catch.n = stockDims)
...
if (sp==1) {
  sr<-window(sr, start = 1963, end = 2009)
