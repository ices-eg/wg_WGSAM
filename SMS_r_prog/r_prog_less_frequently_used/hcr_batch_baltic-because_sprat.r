...
first.year.in.mean<-2036    # years where output is independent of initial conditions and used as "equilibrium"
...
if (do.simulation) {
  ...
  for (CODF in (codF)) for (CLUF in (ClupoidF)) {
      ...
      #HCR@constant.F[2]<-BaseF[2]*CLUF*0.5      #herring
      HCR@constant.F[2]<-0.30      #herring
...
if (do.plots) {
  ...
  by(b,list(b$Species.n),function(x) {
    ...
    i<<-i+1
    contour(xx,y,z,xlab="Cod F",ylab="Sprat F",nlevels = 10)
