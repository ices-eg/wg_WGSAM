...
first.year.in.mean<-2019    # years where output is independent of initial conditions and used as "equilibrium"
last.year.in.mean<-2023
...
if (do.simulation) {
  ...
  HCR@no.MCMC.iterations<-50   #no of iterations
  HCR@last.prediction.year<-2020 
  
  targetFs<-seq(0.18,0.18,1.0)
  Freductions<-seq(15,45,15)
  #Freductions<-50
  ...
  for (targetF in (targetFs)) for (Freduction in (Freductions)) for (TACconstraint in (TACconstraints)) for (Trigger2 in (Trigger2s)){
      ...
      #run SMS
      shell(paste( file.path(root,data.path,"program","sms.exe"),"-mceval",SMS.option,sep=" "), invisible = TRUE)
...
if (read.detailed) {
  ...
  sp.name<-control@species.names
  Years<- c(2008,2009,2010,2012,2014,2030)
...
if (do.plot.detailed) {
 res<-subset(b2,(Year %in% c(2008,2009,2010,2012,2014,2030)))

 a<-subset(res,Year>2008 & TACconstraint==35,select=c(yield,targetF,Freduction,Year,Trigger2,TACconstraint))
 a<-aggregate(a,list(a$targetF,a$Freduction,a$Year,a$Trigger2),mean)
 resTAC<-data.frame(a,Year.fac=as.factor(a$Year),targetF.fac=as.factor(a$targetF), 
  ...
  print(contourplot(yield/1000 ~ targetF * Freduction | Year.fac*Trigger2.fac, data = resTAC, 
              cuts = 10, region = TRUE,at=c(seq(0,1000,100),2000),
              xlab = "F target",
