...
See [here](https://sgaichas.github.io/poseidon-dev/PerCapitaConsumption.html) for the calculation above for NOBA and CCA.
...
This is output from a new [`atlantisom::calc_pred_cons()`](https://github.com/r4atlantis/atlantisom/blob/master/R/calc_pred_cons.R) function based on the Atlantis `PROD.nc` output. This will be incorporated into `atlantisom::run_truth()` when assumptions about output units etc, can be confirmed.

```{r PRODcons, eval=FALSE}
...
We can compare true consumption at age with sampled if we can figure out how to sample it. This is true consumption for intake for now.

```{r trueintake, eval=FALSE}

truenumsagecl <- omlist_ss$truenums_ss %>%
  dplyr::group_by(species, time, agecl) %>%
  dplyr::mutate(totNagecl = sum(atoutput)) #over layer and polygon
...
# ncread <- tidync::tidync(paste0(d.name,"/", nc_out))
# 
# nums3 <- tidync::hyper_tibble(ncread) %>%
#                 dplyr::filter(ncread$variable$name %in% "Long_rough_dab1_Nums")
# 
...
#fleets <- unique(NOBAom_ms$truecatchage_ss$fleet)
# Apr 2021 lets just have one fleet for testing
fleets <- "allcombined"
