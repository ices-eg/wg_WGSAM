...
avgintake <- intakeatlen %>%
  ...
  dplyr::summarise(avgintake = mean(intakesize)) %>%
  tidyr::pivot_wider(names_from = sizebin, values_from = avgintake)

# need to order sizebins

write.csv(avgintake, paste0(o.name,"intake_stomachContent_NOBA.csv"))
