...
We will use a subset of the full NOBA atlantis output, leaving out the model spin up period and the projection period at the end of the run. The goal will be a 60 year dataset for model testing, starting before the onset of fishing and including all of the F contrast built in.
...
fallsacc30 <- ggplot(fallcensus_btempindex, aes(x=year, y=meantemp)) + 
  geom_line() + 
  labs(title = "Fall mean bottom temperature NOBA sacc_30")

springsacc30 <- ggplot(springcensus_btempindex, aes(x=year, y=meantemp)) + 
  geom_line() + 
  labs(title="Spring mean bottom temperature NOBA sacc_30")

fallsacc30 + springsacc30
...
write.csv( , paste0(o.name,"assessmentThresholdsSpecies_NOBA.csv"))

#species column names no rowname all 1s
write_csv( , paste0(o.name,"observation_effortScaling_NOBA.csv"))
...
Initial N at true age can be pulled directly from Atlantis, but we need N at length, which only comes from survey sampling. Here we will use the first survey observation. However this includes NA values and I don't know how well this works with the initial values in the .pin file. 
...
write.csv(startNlen, paste0(o.name,"observation_Y1N_NOBA.csv"))
...
avgrec <- NOBAom_ms$YOY_ss %>% #wide dataset with code as column headings and *duplicate entries*
  #colMeans()/5 #approximation with 5 output timesteps per year
  pivot_longer(-Time, names_to = "Code", values_to = "rec") %>%
  mutate(Code = gsub("\\.0", "", Code)) %>%
  left_join(rectiming) %>%
  if(Time%%365==0){
    mutate(timerecstart = Time+recstart,
         timerecend = Time+recend)
  }else{
    mutate(timerecstart = recstart,
         timerecend = recend)
  }
...
write.csv(recdev, paste0(o.name,"RecDevsPinData_NOBA.csv"))
